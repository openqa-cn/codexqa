...
| `code-wiki` 架构 Wiki 工作流 | 已发布路由契约、playbook、DeepWiki 风格 HTML 报告模板（侧栏 + 正文 + 本页目录，深色 Claude chrome）和已知边界文档；`wiki inputs` / `wiki --no-llm` 依赖同一套 Node.js 18+ 闭源引擎 | 本仓库 CI 不安装或执行该引擎；没有公开宿主 Agent 运行；社区是 Leiden 切分加页数上限，不是模块边界的证明 |
