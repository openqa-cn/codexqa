...
- **Repository renamed from `openqa-skills` to `codexqa`.** The install path is now `npx skills add openqa-cn/codexqa`; badges, links, citation metadata, and documentation titles use the new name. The old GitHub URL redirects. GitHub About now names `codexqa` and its code-graph core, and topics traded three low-signal terms (`developer-tools`, `verification`, `code-graph`) for `sast`, `code-analysis`, and a restored `agentskills`.
