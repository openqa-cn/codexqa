...
- **README follows a shorter developer-tool path.** It keeps one sample report as visual proof, then moves from the six-skill map to installation and two first-success paths. Detailed defect-detection methodology, the full language matrix, extra screenshots, and per-skill evidence remain linked from the skill docs, support matrix, examples, and benchmark pages instead of being repeated in the root README.
