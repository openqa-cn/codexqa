...
- **The README tagline now covers all five skills.** It previously described a PRD as something to *turn into* test cases, which contradicted `requirements-analyzer` — the skill that exists because a PRD may itself have gaps and conflicts. The new line leads with the work after an agent writes code and names requirement defects, code review, PRD gaps, manual cases, and test data. Both language versions say the same thing now.
