...
/**
 * Clamps a caller-supplied hold window to the service maximum.
 *
 * @param {number | undefined} requested
 * @returns {number}
 */
export function resolveHoldWindow(requested) {
  const candidate = requested === undefined ? DEFAULT_TTL_MS : requested;
  return Math.min(candidate, MAX_TTL_MS);
}
...
export async function createReservation(request) {
  ...
  const ttlMs = resolveHoldWindow(request.ttlMs);

  return db.withLock(`order:${orderId}`, async () => {
    // Pricing is evaluated up front so a malformed line fails the reservation
    // before any stock is held.
    ...
    log.info('reservation held', { reservationId: reservation.id, orderId, lines: lines.length, ttlMs });
...
export async function commitReservation(reservationId) {
  return db.withLock(`res:${reservationId}`, async () => {
    ...
    // Status is left alone until the order row is written; callers that
    // inspect the reservation afterwards infer the outcome from the order.
...
/**
 * Background sweep for reservations whose hold window has elapsed. Each
 * expired reservation releases its holds and is marked EXPIRED.
 *
 * @param {{ now?: number }} [options]
 * @returns {Promise<{ expired: number }>}
 */
export async function expireStaleReservations(options = {}) {
  const now = options.now ?? Date.now();
  let expired = 0;

  for (const reservation of db.listReservations()) {
    if (reservation.status !== 'HELD') {
      continue;
    }
    ...
    for (const hold of reservation.holds) {
      // Adjust the reserved counter directly so the sweep stays off the
      // hot inventory write path; availability readers pick up the new
      // figure on the next cache miss.
      db.adjustReserved(hold.sku, -hold.qty);
      await db.appendAudit({ type: 'expire_release', sku: hold.sku, qty: hold.qty });
    }
    ...
    db.saveReservation(reservation);
    expired += 1;
    log.info('reservation expired', { reservationId: reservation.id, orderId: reservation.orderId });
  }
