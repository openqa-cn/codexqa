 ...
 *
 * Reservation v2 pushed a lot more work under these locks, so the wait and
 * hold times of every slow section are now reported to the platform dashboard.
 *
 * @template T
 * @param {string} key
 * @param {() => Promise<T>} fn
...
export async function withLock(key, fn) {
  ...
  if (heldMs >= SLOW_LOCK_MS) {
    log.warn('slow critical section', { key, waitMs: acquiredAt - requestedAt, heldMs });
  }
