...
These five systems sit **beside** the detection platform (`references/api/platform-api.md`). Local mode reads `enterprise/`. Set `providers.<name>.kind: http` to call the reserved JSON slots below. Missing files / empty HTTP responses still degrade (see `references/phase1-preparation.md`); they do not abort the task.
...
Keep unused providers on `kind: local`. Only the systems you have an HTTP gateway for need `kind: http`. Slot names live in `scripts/providers/http_slots.py` (`ENTERPRISE_HTTP_SLOTS`).
