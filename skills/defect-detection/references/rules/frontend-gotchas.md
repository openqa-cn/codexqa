...
> When `service[].language` is `javascript` or `typescript` (or a `.js`/`.ts`/`.vue` file is in a polyglot change set), the Agent should read this file before entering Phase 2 STEP C.
> It covers Vue/React UI **and** plain Node/JS services (Express, Koa, Nest, CLI, workers). Backend Java projects can ignore this file.
...
See `references/phase1-preparation.md#step-1.3a`.
...
node scripts/detect.ts get-pending --batch-id <parentBatchId> --group-by-class
```
...
## 1. AST scan (Step 2.1) — run for JS/TS; skip GitNexus only

> **JS/TS still run AST scan.** `run-ast-scan` loads `AST-JS-*` (eval / `new Function`, `innerHTML` / `dangerouslySetInnerHTML`, `child_process.exec`, query concat, path join, hardcoded secrets, loose `==` (`AST-JS-EQ-001`), empty `catch`, assignment-in-condition, `setTimeout` string, `document.write`, `rejectUnauthorized: false`, taint SSRF `AST-JS-013`, taint open redirect `AST-JS-014`, zero-boundary `AST-JS-BOUND-001`). TypeScript projects keep those JS seeds (`languages: [javascript, typescript]`) and also load `AST-TS-*` (strictness bypass on sinks). GitNexus is skipped on JS/TS **clients**; AST is not. Rules of other languages are filtered, never the whole scan. Do not treat an empty AST result as "no defects" — Hard rule 5 still requires `astRuleCount` on the strategy=8 write-back.
...
**Never hard-code ` ```java `**. `scripts/validate.ts` does not validate code-block language tags, but report readers will be confused by a mismatched tag.
...
Frontend / Node business-detection Call-chain analysis differs from Java backend:
- UI apps: do not follow dependency-package / RPC Call chains. Use **module-import analysis**: `Component → Hook → API Service → Store`
- Plain Node/JS: follow `route → service → repo` (or `handler → lib`) the same way, still using `A#name(L12)→B#name(L45)`; A is the extension-less file path (`src/routes/order`)
- thinking chain format (UI): `ComponentA(L12) → useCustomHook(L34) → api.fetchData(L56) → store.setData(L78)`
- fileCodes includes every file on the chain (.js/.jsx/.mjs/.cjs/.vue/.tsx/.ts), not only a backend-style Controller→Service→Dao
- GitNexus is skipped for JS/TS client repos; always grep-fallback. AST still runs (see section 1)

---

## TypeScript subsection — strictness bypass is not a sanitizer

A type assertion or error suppression does **not** make the value safe. In STEP C, treat these as missing sanitizers, not as evidence the sink is trusted:

- `as any` or `as unknown as T` on `eval` / `innerHTML` / `exec` / `fetch` / `URL` (AST-TS-001, AST-TS-004, AST-TS-007)
- Non-null `!` on `req.query` / `req.body` / `req.params` (AST-TS-002). Same class as an unguarded nullable read
- `// @ts-ignore` or `// @ts-expect-error` immediately above a sink (AST-TS-003)
- `eval($x as any)` / `new Function($x as any)` (AST-TS-005; also covered by AST-JS-001)
- `JSON.parse(...) as any` then a sink (AST-TS-006)

`dangerouslySetInnerHTML` is AST-JS-002 (dual-language JS/TS seed). Do not dismiss an XSS/injection hit because TypeScript compiled.

Agent-only Node/TS patterns (no seed): unhandled promise rejection in `async` route handlers (no `try`/`next(err)`); `await` inside `forEach`; floating promises; `process.env.X` used without a default or validation; `JSON.parse` on request bodies without a schema; regex built from user input (ReDoS); `Buffer.allocUnsafe` leaking memory; prototype pollution via recursive merge of `req.body`; `res.send` after `res.json` (double response)
