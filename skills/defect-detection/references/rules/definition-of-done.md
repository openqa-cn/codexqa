...
- Phase 2 exit: all 14 self-review items OK (per-method writeback, Call chain expansion, full coverage of chain groups, etc.; see `references/phase2-detection.md`).
- When `HAS_CASES=true` or `HAS_DOC=true`, `cross-view-check` should have been run as a **keyword first pass**. It is not a close gate: leftover keyword misses must not block Phase 3, and must not appear in the user summary until semantically confirmed.
...
> ℹ️ After a successful `complete-task` (not `--force`) converge, it **automatically runs `reconcile-report` once** and writes `_reconcile_status` (`CONSISTENT` / `DRIFT`) into the returned JSON. `DRIFT` means section B has not passed. You must fix writeback and re-check with `reconcile-report`. Do not close early. See `references/phase3-close.md` → Step 3.3 / Step 3.4c.
