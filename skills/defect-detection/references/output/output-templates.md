...
Two different surfaces — do not mix them:

| Surface | Audience | Format |
|---|---|---|
| Chat reply after close | Human in the chat | The Markdown template below (tables OK) |
| `complete-task --summary` / `update-summary` / `validate-summary` | Platform report HTML | **Flat plain-text only** — see `summary-spec.md`. Emoji+`[Section]` markers separated by 4 spaces. **Markdown tables are rejected.** |

Authoritative `--summary` rules: `references/output/summary-spec.md`. Always `validate-summary` before finalize.
