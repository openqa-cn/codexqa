...
for class_name, method_name, strategy_code, thinking in METHODS_TO_WRITE:
    ...
    #    Equivalent CLI: node scripts/detect.ts resolve-writeback-context \
    ...
    if not ctx["resolved"]:
        # plan has no filePath or services has no git → do not guess! Rebuild the plan first
        # (clone-and-diff --with-plan / build-detection-plan) or confirm the service is registered
        # (submit-git), or the mapping will be wrong.
