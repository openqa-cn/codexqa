...
> This file contains the detailed bash scripts, batch-writeback strategy, and cross-repo handling for each references/phase2-detection.md step.
...
- [Auto-fix before writeback](#auto-fix-before-writeback)
- [Step 2.0-pre: filter non-business files (run first)](#step-20-pre-filter-non-business-files-run-first)
- [Step 2.0: cross-repo dependency handling](#step-20-cross-repo-dependency-handling)
- [Step 2.1: AST scan](#step-21-ast-scan)
- [Batch-writeback best practices](#batch-writeback-best-practices)
- [Step 2.2: process lifecycle records](#step-22-process-lifecycle-records)
- [Step 2.4: finalize-rank](#step-24-finalize-rank)
- [Phase 3: validation and converge commands](#phase-3-validation-and-converge-commands)
...
- **Language auto-filter**: rules are kept when their Semgrep `languages:` intersect the languages of the scanned files (`languages` in the output); TypeScript also takes JS rules, C++ also takes C rules. Seed packs exist for all 10 supported languages (`references/rules/language-mapping.md`)
- **Semgrep-version resilience**: a rule the installed Semgrep rejects is dropped and remembered (`droppedRules`, `data/semgrep-rejected-rules.json`); a pack that stays invalid returns `scanOk=false` — never "0 hits"
- **Exclusion rules auto-associated**: each finding carries an `exclusionRules` field

## Step 2.1b: Optional overlays

```bash
node "$SKILL_SCRIPT" run-optional-overlays --code-dir <local repo> --task-id $TASK_ID
# --kinds secret,sca,native   --tools bandit,gosec,cppcheck,eslint   --languages python,go   --scan-purpose trunk --full-repo
```

- Registry: gitleaks (secret), trivy | grype (SCA), bandit / ruff / gosec / go vet / staticcheck / cppcheck / eslint / detekt (native, per language of the diff)
- Missing binaries → `skipped[]` + warning, exit 0. Findings share the AST shape plus `tool`, `kind`, `language`; merge with `run-ast-scan` output (`references/phase2-detection.md#step-21b-optional-analyzer-overlays-after-ast-before-write-back`)
...
> For whole classes that are out of scope (generated code, DTOs, test fixtures) use `dismiss-class --task-id ... --batch-id ... --class-names a,b,c` — it marks every planned method of those classes as no-defect in one call; it is not a substitute for method-level analysis of business classes.
...
```bash
echo '$JSON_ARRAY' > "${DETECTION_DATA_DIR:-$SKILL_DIR/data}/_tmp/batch_items.json"
node "$SKILL_SCRIPT" batch-update-process --items-json "${DETECTION_DATA_DIR:-$SKILL_DIR/data}/_tmp/batch_items.json"
```
