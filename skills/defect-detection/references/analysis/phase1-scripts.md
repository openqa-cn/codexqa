...
> This file contains the detailed bash scripts, templates, and degrade strategies for each references/phase1-preparation.md step.
...
- [Step 1.1: locate the script](#step-11-locate-the-script)
- [Step 1.3b: code-permission precheck script](#step-13b-code-permission-precheck-script)
- [Step 1.4: submit the detection task](#step-14-submit-the-detection-task)
- [Step 1.5: register services into content.json](#step-15-register-services-into-contentjson)
- [Step 1.6: parallel data-fetch command list](#step-16-parallel-data-fetch-command-list)
- [Step 1.7: how to read documents](#step-17-how-to-read-documents)
- [Step 1.8: write the detection plan into content.json](#step-18-write-the-detection-plan-into-contentjson)
- [Multi-service orchestration model](#multi-service-orchestration-model)
...
SKILL_SCRIPT="${SKILL_DIR:-${baseDir:-}}/scripts/detect.ts"
if [ ! -f "$SKILL_SCRIPT" ]; then
  SKILL_SCRIPT="$(cd "$(dirname "$0")" && pwd)/scripts/detect.ts"
fi
if [ ! -f "$SKILL_SCRIPT" ]; then
  for base in \
      "$HOME/.cursor/skills" \
  ...
  do
    [ -d "$base" ] || continue
    candidate=$(find "$base" -path "*/defect-detection*/scripts/detect.ts" 2>/dev/null | head -1)
    if [ -n "$candidate" ]; then SKILL_SCRIPT="$candidate"; break; fi
  done
fi
if [ ! -f "$SKILL_SCRIPT" ]; then
  SKILL_SCRIPT=$(find "$HOME" -name detect.ts -path "*defect-detection*/scripts/*" -not -path "*node_modules*" 2>/dev/null | head -1)
fi
```
...
>
> **Java Call chain (default)**: after a successful clone, `ensure-gitnexus` (probe → force-install once if missing → `gitnexus analyze`). See `references/phase1-preparation.md` Step 1.2b. Degrade to grep only if install fails and cannot be repaired.
...
# Clone root = $DETECTION_CLONE_DIR, else <data dir>/repos (data dir = $DETECTION_DATA_DIR or <skill>/data)
LOCAL_DIR="${DETECTION_CLONE_DIR:-${DETECTION_DATA_DIR:-$SKILL_DIR/data}/repos}/${REPO_HASH}"
...
# 2. Write Git Diff
node "$SKILL_SCRIPT" set-diff-files --task-id $TASK_ID --files-json "$DIFF_FILES_JSON"          # JSON array of paths
node "$SKILL_SCRIPT" set-changed-methods --task-id $TASK_ID --class-name "$CLASS" --methods-json "$METHODS_JSON"  # one call per class

# 3. Write rules / tags / historical defects (--value is parsed as JSON when possible, otherwise stored as a string)
node "$SKILL_SCRIPT" set-meta-field --task-id $TASK_ID --key "rules" --value "$RULES_JSON"
node "$SKILL_SCRIPT" set-meta-field --task-id $TASK_ID --key "tags" --value "$VALID_TAG_IDS_JSON"
node "$SKILL_SCRIPT" set-meta-field --task-id $TASK_ID --key "historyDefects" --value "$HISTORY_DEFECTS_JSON"
```
...
```bash
# Method 1 (prefer): the configured docs provider. `phase1-fetch-all` / `add-document`
# call it automatically for any doc given only as an id / URL / file stem:
#   - local  (default): enterprise/docs/<stem>.md|.txt, or a plain http(s) GET
#   - http   (DETECTION_DOCS_KIND=http): the `docs.fetch` slot of your platform
#     (see references/api/enterprise-http-api.md)
# Result lands in the doc record as content / fetchStatus / fetchMethod / fetchError.

# Method 2 (degrade): the host agent's own document/search tool, then register with
#   node "$SKILL_SCRIPT" add-document --task-id $TASK_ID --doc-type techDocs --doc-json '{...,"content":"..."}'
# Method 3 (last resort): web_fetch the URL and register the same way
```
...
| Scene | Handling |
|---|---|
| docs provider returns `fetchStatus: failed` (auth / 404 / not in enterprise/docs) | Degrade to the host agent's document/search tool |
| Host tool has no results | Degrade to web_fetch |
