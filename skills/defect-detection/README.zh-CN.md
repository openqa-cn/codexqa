...
`defect-detection` 会克隆 git 仓库和分支、提取变更方法、运行静态规则与可选调用图，再让 Agent 对照需求和测试用例判断实现是否偏离意图。疑似缺陷必须通过结构化写回规则和关门闸，才能进入报告。输入是 git 仓库和分支（再加你能提供的需求或用例）；本地 CLI、静态规则和可选适配器均可独立使用。工作流能跑；检出效果还没有独立 benchmark。
