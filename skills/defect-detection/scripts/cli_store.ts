...
import { _cli_error, _cli_success, _parse_json_arg, _require_save } from "./cli_common.ts";
import { _rule_paragraphs_from_doc, restamp_plan_relevance } from "./cli_auto.ts";
import { fetch_document_body, service_local_dirs } from "./ingest.ts";
import { canonicalize_language } from "./lang.ts";
import { ContentStore, changed_method_name } from "./store.ts";
...
export function cmd_add_document(args: Args): void {
  ...
  // Plan JSON speaks requirementDocs / technicalDocs; the store speaks prdDocs / techDocs.
  args.doc_type = ({ requirementDocs: "prdDocs", technicalDocs: "techDocs" } as Record<string, string>)[args.doc_type] || args.doc_type;
  // Fill content: in-repo file from the registered clone, else the configured docs provider.
  fetch_document_body(doc, service_local_dirs(store));
  try {
  ...
  // Relevance matching reads `extractedRules`, not raw documents. Without a seed rule
  // here, every plan item stays detectTier=T3 after `clone-and-diff --with-plan` and the
  // context-read gate is silently exempt — even when the agent did register the doc.
  // Seed one rule per paragraph (same splitter as extract-rules-from-docs, so a later
  // run dedupes by text) rather than the whole body as a single 20k-char "rule".
  const body = String(doc.content || doc.businessRules || "").trim();
  let restamped = 0;
  let upgraded: string[] = [];
  let seeded_rules = 0;
  if (body.length >= 20) {
    const rules = store.get_extracted_rules().slice();
    const source = String(doc.contentId || doc.title || args.doc_type);
    const existing_texts = new Set(rules.map((r: any) => String(r?.text || "")));
    const paragraphs = _rule_paragraphs_from_doc(body);
    // A body with no sentence-like paragraph (e.g. one long line) still seeds itself.
    const candidates = paragraphs.length ? paragraphs : [body.slice(0, 500)];
    for (const text of candidates) {
      if (existing_texts.has(text)) continue;
      existing_texts.add(text);
      rules.push({ text, source, related: String(doc.title || "") });
      seeded_rules += 1;
    }
    if (seeded_rules) store.set_extracted_rules(rules);
    try {
      const r = restamp_plan_relevance(store);
      restamped = r.updated;
      upgraded = r.upgraded;
    } catch {
      // plan may not exist yet; check-phase2-readiness will restamp later
    }
  }

  _require_save(store);
  const fetched = doc.fetchStatus ? ` (fetchStatus=${doc.fetchStatus}${doc.fetchMethod ? `, via ${doc.fetchMethod}` : ""})` : "";
  _cli_success(
    {
      ...
      contentLength: String(doc.content || "").length,
      extractedRuleSeeded: seeded_rules > 0,
      extractedRulesSeededCount: seeded_rules,
      planRestamped: restamped,
      tierUpgraded: upgraded,
    },
    `${args.doc_type} document added${fetched}` +
      (restamped ? `; re-stamped ${restamped} plan item(s)` : ""),
  );
}
...
export function cmd_add_test_case(args: Args): void {
  ...
  // Common aliases agents use when they have not opened the schema.
  if (caseObj.caseId != null && caseObj.id == null) caseObj.id = caseObj.caseId;
  delete caseObj.caseId;
  if (caseObj.description != null) {
    if (caseObj.steps == null) caseObj.steps = caseObj.description;
    if (caseObj.expectedResult == null) caseObj.expectedResult = caseObj.description;
    delete caseObj.description;
  }
  if (caseObj.expected != null && caseObj.expectedResult == null) {
    caseObj.expectedResult = caseObj.expected;
    delete caseObj.expected;
  }
  // Upsert by id: a second call for the same case (e.g. adding semantic
  // compilation fields after the body) merges into the existing record instead
  // of appending a duplicate that would leave a half-filled copy behind.
  const existed = caseObj.id != null && store.get_test_cases().some((c: any) => c.id === caseObj.id);
  try {
    store.upsert_test_case(caseObj);
  } catch (e: any) {
    _cli_error(`data validation failed: ${e.message || e}`);
  }
  _require_save(store);
  _cli_success({ id: caseObj.id, updated: existed }, existed ? "test case updated" : "test case added");
}
