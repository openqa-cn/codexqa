#!/usr/bin/env node
/**
 * tags.ts — persist the platform tag catalog to local JSON files.
 *
 ...
 * Usage:
 *     node tags.ts
 *     node tags.ts --output-flat ./tags.json
 */
...
function _parse_cli(argv: string[]): Record<string, any> {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (tok === "-h" || tok === "--help") {
      console.log(
        "usage: tags.ts [-h] [--output-flat OUTPUT_FLAT] [--output-tree OUTPUT_TREE] [--dry-run]\n\n" +
      ...
      const raw = argv[++i];
      if (raw === undefined) {
        console.error("tags.ts: error: option --output-flat requires an argument");
        process.exit(2);
      }
      out.output_flat = raw;
      ...
      const raw = argv[++i];
      if (raw === undefined) {
        console.error("tags.ts: error: option --output-tree requires an argument");
        process.exit(2);
      }
      out.output_tree = raw;
    } else if (tok === "--dry-run") {
      ...
      console.error(`tags.ts: error: unrecognized arguments: ${tok}`);
      process.exit(2);
    }
  }
  return out;
