#!/usr/bin/env node
/**
 * lessons.ts — silent lessons-learned detection and storage
 *
 * Responsibilities:
 ...
 *
 * Usage:
 *     node lessons.ts detect --task-id 42       # fully silent, empty stdout
 *     node lessons.ts peek --category phase1     # return Top-N lessons JSON
 *     node lessons.ts report --days 7            # Markdown frequency stats
 *
...
const LESSONS_FILE = join(SKILL_DIR, "references", "output", "lessons-learned.md");
...
function _parse_int(raw: string, flag: string): number {
  ...
  if (Number.isNaN(n)) {
    console.error(`lessons.ts: error: argument ${flag}: invalid int value: '${raw}'`);
    process.exit(2);
  }
  return n;
}
...
export function main(argv: string[] = process.argv.slice(2)): void {
  ...
  if (!cmd || cmd === "-h" || cmd === "--help") {
    console.log(
      "usage: lessons.ts [-h] {detect,peek,report} ...\n\n" +
  ...
  if (cmd === "detect") {
    ...
    for (let i = 1; i < argv.length; i++) {
      ...
      if (tok === "--task-id") {
        const raw = argv[++i];
        if (raw === undefined) {
          console.error("lessons.ts: error: option --task-id requires an argument");
          process.exit(2);
        }
        ...
        console.log(
          "usage: lessons.ts detect [-h] --task-id TASK_ID [--emit]\n\n" +
        ...
        console.error(`lessons.ts: error: unrecognized arguments: ${tok}`);
        process.exit(2);
      }
    }
    if (task_id === undefined) {
      console.error("lessons.ts: error: the following arguments are required: --task-id");
      process.exit(2);
    }
    detect(task_id, emit);
  } else if (cmd === "peek") {
    ...
    for (let i = 1; i < argv.length; i++) {
      ...
      if (tok === "--category") {
        const raw = argv[++i];
        if (raw === undefined) {
          console.error("lessons.ts: error: option --category requires an argument");
          process.exit(2);
        }
        category = raw;
      } else if (tok === "--limit") {
        const raw = argv[++i];
        if (raw === undefined) {
          console.error("lessons.ts: error: option --limit requires an argument");
          process.exit(2);
        }
        limit = _parse_int(raw, "--limit");
      } else if (tok === "-h" || tok === "--help") {
        console.log("usage: lessons.ts peek [-h] [--category CATEGORY] [--limit LIMIT]");
        process.exit(0);
      } else {
        console.error(`lessons.ts: error: unrecognized arguments: ${tok}`);
        process.exit(2);
    ...
    for (let i = 1; i < argv.length; i++) {
      ...
      if (tok === "--days") {
        const raw = argv[++i];
        if (raw === undefined) {
          console.error("lessons.ts: error: option --days requires an argument");
          process.exit(2);
        }
        days = _parse_int(raw, "--days");
      } else if (tok === "-h" || tok === "--help") {
        console.log("usage: lessons.ts report [-h] [--days DAYS]");
        process.exit(0);
      } else {
        console.error(`lessons.ts: error: unrecognized arguments: ${tok}`);
        process.exit(2);
