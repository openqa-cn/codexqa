...
export class LocalPlatformProvider extends BasePlatformProvider {
  ...
  _ensure_seed(): void {
    ...
    const stored_rules = existsSync(rules_path) ? this.store.read("rules.json", { default: {} }) : null;
    if (!stored_rules || is_legacy_rule_catalog(stored_rules)) {
      this.store.write("rules.json", { data: DEFAULT_RULES });
    } else if (!stored_rules.userModified && (stored_rules.seedVersion || 1) < SEED_CATALOG_VERSION) {
      // Refresh an untouched copy when the shipped seed catalog changed (new / fixed Semgrep rules).
      this.store.write("rules.json", { data: DEFAULT_RULES });
      console.error(
        `[local-platform] 🔄 rules.json refreshed to seed catalog v${SEED_CATALOG_VERSION} ` +
          "(set \"userModified\": true in data/platform/rules.json to keep your own edits)",
      );
    }
  ...
  write_html_report(task_id: number): string {
    ...
    // Keep user-confirmed (3) and deferred (5) findings visible alongside AI 6/7.
    // Exclude false-positive (4) and duplicate (8).
    const defects = ranks.filter((r) => [3, 5, 6, 7].includes(r.bugStatus) && !r.invalid);
  ...
  override submit_detection(request_body: Record<string, any>): Record<string, any> {
    ...
    if (!services.length && request_body.git) {
      services = [{
        ...
        language: request_body.language || null,
      }];
    }
    ...
    if (!services.length) {
      services = [{ git: "", branch: "", serviceKey: "default", language: null }];
    }
  ...
  override check_detection_coverage(batch_id: number): Record<string, any> {
    ...
    return ok({
      ...
      // Aliases matching the remote platform contract (complete-task reads these).
      totalCount: processes.length,
      completedCount: processes.length - missed.length,
  ...
  /**
   * Seed pending (bugStatus=0) processes for detection-plan items so the local
   * platform mirrors the remote contract: `get-pending` lists the work and
   * `check-coverage` counts every planned method until it is written back.
   * Idempotent: an existing process with the same class/method/strategy is reused.
   */
  override seed_pending_processes(batch_id: number, items: Record<string, any>[]): Record<string, any> {
    const batch = this._batch(batch_id);
    if (!batch) return fail(`batch ${batch_id} not found`);
    const existing = this._processes_for_batch(batch_id);
    const seeded: Record<string, any>[] = [];
    const reused: Record<string, any>[] = [];
    const batch_ids = [...(batch.processIds || [])];
    for (const item of items || []) {
      const class_name = item.className;
      if (!class_name) continue;
      const method_name = item.methodName || "";
      const strategy = item.strategyCode ?? 11;
      const match = existing.find((p) =>
        p.className === class_name && (p.methodName || "") === method_name && p.strategyCode === strategy,
      );
      if (match) {
        reused.push({ processId: match.processId, className: class_name, methodName: method_name, strategyCode: strategy });
        continue;
      }
      const record: Record<string, any> = {
        processId: this._next_id("nextProcessId"),
        parentBatchId: batch_id,
        batchId: batch_id,
        taskId: batch.taskId,
        ...
        detectTier: item.detectTier || null,
        bugStatus: 0,
        seededFrom: "detection-plan",
        createdAt: _now(),
        updatedAt: _now(),
      };
      this._save_process(record);
      existing.push(record);
      batch_ids.push(record.processId);
      seeded.push({ processId: record.processId, className: class_name, methodName: method_name, strategyCode: strategy });
    }
    if (seeded.length) {
      batch.processIds = batch_ids;
      this._save_batch(batch);
    }
    return ok({ batchId: batch_id, seeded, reused, seededCount: seeded.length, reusedCount: reused.length });
  }

  override update_process(request_body: Record<string, any>): Record<string, any> {
    const process_id = request_body.processId;
    if (process_id === null || process_id === undefined) {
      // An explicit `processId: null` from a plan item must not wipe the id of the
      // process we match below by class/method/strategy.
      request_body = { ...request_body };
      delete request_body.processId;
    }
  ...
  override check_rank_integrity(batch_id: number): Record<string, any> {
    ...
    // Shape matches the remote contract that finalize-all consumes: classPath,
    // aggregatedBugStatus and the defect processes whose content gets merged into the rank.
    const missing_rank = [...process_keys].filter((k) => !rank_keys.has(k)).sort().map((k) => {
      const [c, m] = JSON.parse(k);
      const defect_processes = processes.filter((p) =>
        p.className === c && (p.methodName || "") === m && (p.bugStatus === 6 || p.bugStatus === 7),
      );
      // A suspected defect (6) outranks an improvement (7) when both exist on one method.
      const aggregated = defect_processes.some((p) => p.bugStatus === 6) ? 6 : 7;
      return {
        className: c,
        classPath: c,
        methodName: m,
        aggregatedBugStatus: aggregated,
        defectProcesses: defect_processes.map((p) => ({
          processId: p.processId,
          strategyCode: p.strategyCode,
          bugStatus: p.bugStatus,
          content: p.content,
        })),
      };
    });
    ...
    return ok({
      ...
      // Aliases matching the remote platform contract (complete-task reads these).
      totalClassCount: new Set(processes.map((p) => p.className)).size,
      rankExistsCount: ranks.length,
    });
  }
  ...
  override get_report(task_id: number): Record<string, any> {
    ...
    const defect_ranks = ranks.filter((r) => [3, 5, 6, 7].includes(r.bugStatus) && !r.invalid);
  ...
  override report_progress(body: Record<string, any>): Record<string, any> {
    ...
    // clone-and-diff learns the real baseline after submit-git (whose --contrast-branch
    // is only a guess); let it refresh the task metadata shown in status / report.
    const task = this._task(Number(task_id));
    if (task) {
      let touched = false;
      for (const key of ["contrastBranch", "contrastCommit", "commitId", "developBranch"]) {
        if (body[key] && task[key] !== body[key]) {
          task[key] = body[key];
          touched = true;
        }
      }
