export interface PlatformProvider {
  ...
  /** Optional: pre-create pending processes for plan items (platforms that own their own process list return `supported: false`). */
  seed_pending_processes(batch_id: number, items: Record<string, any>[]): Record<string, any>;
...
export class BasePlatformProvider implements PlatformProvider {
  ...
  // Remote platforms build their own pending list from the diff; nothing to seed.
  seed_pending_processes(batch_id: number, _items: Record<string, any>[]): Record<string, any> {
    return { code: 0, msg: "not supported by this provider", data: { batchId: batch_id, supported: false, seeded: [], reused: [], seededCount: 0, reusedCount: 0 } };
  }
