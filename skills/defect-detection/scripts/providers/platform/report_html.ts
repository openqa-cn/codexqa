...
function _status_label(status: any): string {
...
function _short_time(value: any): string {
...
function _short_git(git: string): string {
...
function _needs_confirm(text: string): boolean {
...
function _confirm_hints(sections: Array<[string, string]>): Set<string> {
...
function _finding_needs_confirm(item: Record<string, any>, confirm_hints: Set<string> | null = null): boolean {
...
function _parse_summary_sections(summary: string): Array<[string, string]> {
...
function _section_title(heading: string): string {
  const match = heading.match(/\[([^\]]+)\]/);
  return match ? match[1] : heading;
}

function _section_meta(heading: string): [string, string] {
  const title = _section_title(heading);
  return _SECTION_META[title] || ["other", "doc"];
}

function _nav_html(sections: Array<[string, string]>, has_findings: boolean): string {
  const items: string[] = [];
...
function _kpi_html(counts: Record<string, number>, sections: Array<[string, string]>): string {
  let risk_n = 0;
  let confirm_n = 0;
  for (const [heading, body] of sections) {
    const title = _section_title(heading);
...
function _render_summary_sections(sections: Array<[string, string]>): string {
  if (!sections.length) {
...
function _render_section_body(title: string, body: string): string {
...
function _parse_risk_items(body: string): Record<string, string>[] {
  ...
  for (const chunk of chunks) {
    if (chunk === "▎This change" || chunk === "▎Pre-existing") {
      // Flush the item that precedes the sub-heading under its own era first.
      if (buf.trim()) {
...
function _split_risk_item(raw: string, era: string): Record<string, string> | null {
  let text = raw.trim();
...
function _render_risks(body: string): string {
...
function _render_confirms(body: string): string {
...
function _render_notes(body: string): string {
...
function _finding_line_label(content: string): string {
  const [, fields] = _parse_finding_fields(content);
  const line = (fields["Lines"] || "").trim();
  return line ? `Lines ${line}` : "";
}

function _finding_location(item: Record<string, any>): string {
...
function _finding_title(content: string): string {
  const first = content.split(/\r?\n/).map((line) => line.trim()).find((line) => line) || "No details";
  return first.replace(/^(Defect|Improvement)[：:]\s*/, "").slice(0, 80);
}

function _parse_finding_fields(content: string): [string, Record<string, string>, string] {
  let code = "";
  let working = content;
...
function _finding_detail_html(item: Record<string, any>): string {
...
function _render_tags(raw: string): string {
