...
export function prepare_rules(rules_list: Record<string, any>[]): Record<string, any> {
  ...
  const _counts = (m: Record<string, any[]>) =>
    JSON.stringify(Object.fromEntries(Object.entries(m).map(([k, v]) => [k, v.length])));
  console.error(
    `[open_rules] rule grouping complete: ` +
      `category=${_counts(by_category)}, ` +
      `strategy=${_counts(by_strategy)}, ` +
