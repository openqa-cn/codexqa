...
import { get_docs, get_plan, get_traces } from "./providers/registry.ts";
import { existsSync, readFileSync, statSync } from "node:fs";
import { join, resolve, sep } from "node:path";

/** Same cap as scan-repo-docs: keep a single in-repo document from flooding the context. */
const _REPO_DOC_MAX_CHARS = 8000;

/** localDir of every registered service (for in-repo document lookups). */
export function service_local_dirs(store: ContentStore): string[] {
  const meta = store.load_meta_only();
  return ((meta.services || []) as any[]).map((s) => String(s?.localDir || "")).filter(Boolean);
}
...
function _has_items(...groups: any[]): boolean {
  return groups.some((g) => py_bool(g));
}

function _normalize_service(item: any): Record<string, any> | null {
  if (typeof item === "string" && item.trim()) {
    return { git: item.trim(), gitUrl: item.trim(), branch: "" };
  }
  ...
  // A plan that names a language is a declaration; otherwise leave it unset so
  // clone-and-diff can detect it from the changed files instead of assuming Java.
  if (out.language) {
    out.language = canonicalize_language(out.language);
    if (!out.languageSource) out.languageSource = "declared";
  } else {
    delete out.language;
  }
  return out;
...
/**
 * A doc registered as a bare id / URL / file stem has no body yet. Ask the
 * configured docs provider (enterprise/docs/*.md locally, the `docs.fetch` HTTP
 * slot remotely) and merge the result in place. Never throws; failures are
 * recorded as fetchStatus=failed + fetchError so Phase 1 can degrade.
 */
export function fetch_document_body(doc: Record<string, any>, local_dirs: string[] = []): Record<string, any> {
  if (!doc || typeof doc !== "object") return doc;
  if (doc.content || doc.businessRules) return doc;
  const ref = String(doc.contentId || doc.url || doc.filePath || "");
  if (!ref) return doc;

  // In-repo document (`repoSource=local_repo`, or a relative filePath that exists in a
  // registered clone): read it from disk; the enterprise docs provider does not know it.
  const rel = String(doc.filePath || "");
  if (rel && !/^https?:\/\//.test(rel)) {
    for (const dir of local_dirs) {
      const root = resolve(dir) + sep;
      const abs = resolve(dir, rel);
      // Confine reads to the clone — reject `../` escapes.
      if (!abs.startsWith(root) && abs !== resolve(dir)) continue;
      if (existsSync(abs) && statSync(abs).isFile()) {
        doc.content = readFileSync(abs, "utf8").slice(0, _REPO_DOC_MAX_CHARS);
        if (!doc.businessRules) doc.businessRules = doc.content;
        doc.repoSource = doc.repoSource || "local_repo";
        doc.fetchMethod = "local_repo";
        doc.fetchStatus = "success";
        delete doc.fetchError;
        return doc;
      }
    }
    if (doc.repoSource === "local_repo") {
      doc.fetchStatus = "failed";
      doc.fetchError = local_dirs.length
        ? `file not found in registered clone(s): ${rel}`
        : "no registered clone (run clone-and-diff first) to read the in-repo document";
      return doc;
    }
  }

  try {
    const fetched = get_docs().fetch(ref);
    if (fetched && typeof fetched === "object") {
      for (const [k, v] of Object.entries(fetched)) {
        if (v !== undefined && v !== "" && (doc[k] === undefined || doc[k] === "")) doc[k] = v;
      }
      if (fetched.fetchStatus) doc.fetchStatus = fetched.fetchStatus;
      if (fetched.fetchMethod) doc.fetchMethod = fetched.fetchMethod;
      if (fetched.fetchError) doc.fetchError = fetched.fetchError;
    }
  } catch (exc: any) {
    doc.fetchStatus = "failed";
    doc.fetchError = String(exc?.message || exc);
  }
  return doc;
}
...
export function classify_materials(
  ...
  store = store || {};
  cli = cli || {};
  const has_code = py_bool(data.services);
  const has_req = py_bool(data.requirementDocs);
  const has_tech = py_bool(data.technicalDocs);
  const has_cases = py_bool(_py_or(data.testCaseIds, store.testCases, cli.testCases));
  const has_history = py_bool(_py_or(store.historyDefects, cli.historyDefects));
  const has_traces = py_bool(_py_or(store.traces, cli.traces));
