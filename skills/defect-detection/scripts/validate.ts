...
import { has_code_been_read, has_repo_been_cloned } from "./state.ts";
import { guess_file_path_for_class } from "./lang.ts";
...
export function validate_update_process(
  ...
  if (strategy_code === 8) {
    ...
    if (ast_rule_count === 0) {
      return (
        ...
        "Java and JS/TS local seeds always include category=1 rules; astRuleCount=0 is abnormal. " +
  ...
  // Hard rule 6 follows the tier contract (detection-plan-spec): T3 skips STEP B entirely,
  // and a no-defect conclusion only needs chain evidence at T1.
  const _tier = _s(request_body.detectTier);
  const _chain_required = _in(strategy_code, 4, 6, 11) && _tier !== "T3" && (has_defect || _tier === "T1");
  if (_chain_required) {
  ...
  if (_in(strategy_code, 6, 11)) {
    ...
    if (!has_ref_sources) {
      return (
        "Hard rule7 (business strategy referencedSources): when strategyCode is 6 or 11, " +
    ...
    if (has_defect) {
      ...
      // Only require a test-case citation when the task actually has cases.
      // HAS_CASES=false (no plan cases / chat cases) must not force invented IDs.
      // When cases are mapped to methods (relatedMethods / plan caseRelevance), only the
      // methods a case actually covers must cite one — never force an unrelated method to
      // invent a case link. Unmapped cases keep the task-wide requirement.
      let task_has_cases = true;
      if (context_task_id != null && String(context_task_id).trim() !== "") {
        try {
          const store = new ContentStore(Number(context_task_id));
          const cases: any[] = store.get_test_cases();
          const meta_has = store.get_meta_field("hasCases");
          task_has_cases = (Array.isArray(cases) && cases.length > 0) || meta_has === true || meta_has === "true";
          if (task_has_cases && Array.isArray(cases) && cases.some((c) => Array.isArray(c?.relatedMethods) && c.relatedMethods.length)) {
            const cn = _s(request_body.className);
            const mn = _s(request_body.methodName);
            const simple = cn.split(/[/.]/).pop() || cn;
            const covered = cases.some((c) => (c?.relatedMethods || []).some((rm: any) => {
              const ref = String(rm);
              return ref === `${cn}#${mn}` || ref.endsWith(`/${simple}#${mn}`) || ref.endsWith(`.${simple}#${mn}`) || ref === `${simple}#${mn}`;
            }));
            const plan_item = store.get_plan_item(cn, mn, strategy_code);
            const plan_rel = _s(plan_item?.caseRelevance);
            task_has_cases = covered || (plan_rel !== "" && plan_rel !== "none");
          }
        } catch {
          task_has_cases = true; // fail open to previous behavior when store unavailable
        }
      }
      if (task_has_cases && !_CASE_KEYWORDS.some((kw) => content_text.includes(kw))) {
        return (
          "Hard rule7 (business strategy): when strategyCode is 6 or 11, bugStatus=6/7 and the task has test cases, content must include the related test-case information." +
  ...
  if (has_defect && _cached_tag_ids !== null) {
    ...
    if (py_bool(tag_id) && !_cached_tag_ids.has(tag_id)) {
      try {
  ...
  if (has_defect && _cached_tag_map !== null) {
    const tag_id = request_body.tagId;
    const content_text = _s(request_body.content);
    if (py_bool(tag_id) && content_text) {
  ...
  if (!_in(strategy_code, 8)) {
    ...
    if (py_bool(batch_id) && py_bool(class_name)) {
  ...
  if (py_bool(batch_id) && git_url) {
    if (!has_repo_been_cloned(batch_id, git_url, task_id_for_clone_gate)) {
      return (
        ...
        "and register it with register-repo-clone. " +
  ...
  if (py_bool(min_detect_level) && !_in(strategy_code, 8) && !_is_light) {
...
export function pre_validate_and_fix(request_body: Record<string, any>): Record<string, any> {
  ...
  for (let i = 0; i < file_codes.length; i++) {
    ...
    if (!file_path && class_name) {
      // Language comes from the item itself when present; never hardcode java (R8).
      const lang = request_body.language || request_body._language || null;
      const inferred = guess_file_path_for_class(class_name, lang);
