...
# Output: defect-detection.zip  with top-level folder defect-detection/SKILL.md
...
if ! grep -q '^name: defect-detection$' "$DEST/SKILL.md"; then
  echo "SKILL.md name is not defect-detection" >&2
  exit 1
fi
if [ ! -f "$DEST/scripts/detect.ts" ]; then
  echo "missing scripts/detect.ts" >&2
  exit 1
fi
