...
import { render_report_html, _parse_confirm_items } from "../scripts/providers/platform/report_html.ts";
...
describe("render_report_html", () => {
  it("structures risks and findings", () => {
    ...
    const html = render_report_html({
      ...
      status: "completed",
      git: "git@example.com:acme/demo.git",
  ...
  it("unpacks Chinese pipe-packed pending confirmation cards", () => {
    const body = [
      "▸(1) 值域矛盾项: order-service 写入缓存 paymentType 的实际值域是 1/2/3 还是 0/1/2 | 当前假设：写入方与计价侧一致使用 1/2/3，仅注释错误 | 请确认：写入方枚举定义；若确认仅注释问题请修正三处注释。",
      "▸(2) 降级开关项: pricing.degrade 是否为遗漏接线的独立手动开关 | 当前假设：预留未接线 | 请确认：运维手册中两个配置键的预期用途。",
    ].join("");
    const items = _parse_confirm_items(body);
    assert.equal(items.length, 2);
    assert.equal(items[0].title, "值域矛盾项");
    assert.equal(items[0]["Open question"], "order-service 写入缓存 paymentType 的实际值域是 1/2/3 还是 0/1/2");
    assert.equal(items[0]["Current assumption"], "写入方与计价侧一致使用 1/2/3，仅注释错误");
    assert.equal(items[0]["Please confirm"], "写入方枚举定义；若确认仅注释问题请修正三处注释。");
    assert.equal(items[1].title, "降级开关项");
    assert.equal(items[1]["Open question"], "pricing.degrade 是否为遗漏接线的独立手动开关");
    assert.equal(items[1]["Current assumption"], "预留未接线");
    assert.equal(items[1]["Please confirm"], "运维手册中两个配置键的预期用途。");
    ...
    assert.match(html, /<dt>Open question<\/dt><dd>order-service 写入缓存 paymentType 的实际值域是 1\/2\/3 还是 0\/1\/2<\/dd>/);
    assert.match(html, /<dt>Current assumption<\/dt><dd>写入方与计价侧一致使用 1\/2\/3，仅注释错误<\/dd>/);
    assert.match(html, /<dt>Please confirm<\/dt><dd>写入方枚举定义；若确认仅注释问题请修正三处注释。<\/dd>/);
    assert.match(html, /<h3>#2 降级开关项<\/h3>/);
    assert.match(html, /<dt>Open question<\/dt><dd>pricing.degrade 是否为遗漏接线的独立手动开关<\/dd>/);
