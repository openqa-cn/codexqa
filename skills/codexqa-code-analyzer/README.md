...
`codexqa-code-analyzer` answers **structure and impact** questions from graph evidence; it does not decide whether the implementation matches a business requirement or assign P0 / P1 / P2 review findings. Use `codexqa-defect-analyzer` for SAST + Agent LLM Detection code-risk scan reports, `codexqa-code-reviewer` for CodexQA evidence-pack HTML review (heuristic dims + Agent LLM judgment), `codexqa-code-wiki` for architecture wiki, and `codexqa-rootcause-analyzer` for full exception RCA on top of the CodexQA CLI. They can be chained: map the impact here, then focus the deeper review or RCA.
...
| **Cursor** | Put `codexqa-code-analyzer/` in `~/.cursor/skills/` or `.cursor/skills/` | Full QA workflow |
