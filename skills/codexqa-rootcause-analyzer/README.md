...
`codexqa-rootcause-analyzer` turns exception evidence (stack traces, logs, call-chain dumps, debug output) plus a business codebase into an **English** Markdown root-cause report (`report.md` / `report.en.md`). Structured repo understanding comes only from the public **CodexQA CLI** (`@openqa-cn/codexqa`); this skill does not copy engine or skill source.

It is **not** [`codexqa-code-analyzer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-code-analyzer/README.md) (symbol-graph impact / callers / test gaps), **not** [`codexqa-defect-analyzer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-defect-analyzer/README.md) (SAST + Agent LLM Detection code-risk scan reports), and **not** [`codexqa-code-reviewer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-code-reviewer/README.md) (CodexQA evidence-pack HTML review with Agent LLM judgment). Use those for structure, requirements, or review findings; use this skill when the input is an exception and the goal is RCA.
...
|---|---|
| `npx skills add openqa-cn/codexqa --skill codexqa-rootcause-analyzer` | Installs the skill into a coding-agent skills directory |
| `node scripts/diagnose.ts …` | Runs the diagnosis CLI (this is **not** an `npx` binary) |

There is no `npx codexqa-rootcause-analyzer` / package `bin`. After install, point `$SKILL_DIR` at the installed folder and run `node "$SKILL_DIR/scripts/diagnose.ts"`.
