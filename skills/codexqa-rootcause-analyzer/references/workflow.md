...
**Uploaded / non-git strategy (CodexQA):** `codexqa index` requires a git repository. Never `git init` in the user's original folder. `submit` copies the uploaded file, stack-referenced siblings found in that folder, and other **top-level** source files in the same directory into `data/<taskId>/repo`, then `git init` **only in that task copy**. Nested trees (`node_modules`, nested checkouts, `.codexqa-testcase-generator`) are not walked. If the origin already has `.git`, index it in place.
