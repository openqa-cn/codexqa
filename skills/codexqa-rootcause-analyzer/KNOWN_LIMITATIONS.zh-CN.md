...
没有公开答案键 fixture，也没有和 codexqa-defect-analyzer inventory-service 7/7 对等的宿主 agent 成绩。把「写出一份英文报告」当成设计路径，不要当成已测过的 RCA 准确率。

## 工作流边界

本 skill 诊断**异常**。它不替代 `codexqa-code-analyzer` 的变更影响审查、`codexqa-defect-analyzer` 的代码风险扫描，也不替代 `codexqa-code-reviewer` 的 CodexQA 证据包 HTML 评审。
