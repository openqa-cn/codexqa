...
[`codexqa-testdata-generator`](README.md) does not ship a model. It is routing plus scripts on the host agent: the model chooses a path and extracts parameters you already gave; what lands in the backend is an executor / published tool / generated script calling that backend.
...
Agents read [`SKILL.md`](SKILL.md), not this page. Command-level detail is in the [operator appendix](#operator-appendix) below. Gaps: [Known limitations](KNOWN_LIMITATIONS.md). Sample write-back: [preview](https://htmlpreview.github.io/?https://github.com/openqa-cn/codexqa/blob/main/docs/assets/previews/testdata-writeback.html).
...
| Name | Meaning |
|---|---|
| Source directory | `codexqa-testdata-generator/` |
| Install directory / skill name | `codexqa-testdata-generator/` (must match the `name` in `SKILL.md`) |
| `SKILL_DIR` | Directory that contains this skill’s `SKILL.md` (source tree or `~/.cursor/skills/codexqa-testdata-generator`) |
...
```bash
cd codexqa-testdata-generator
node scripts/pack_skills.ts --output ./dist
unzip dist/codexqa-testdata-generator.zip -d ~/.cursor/skills
```

Artifact: `dist/codexqa-testdata-generator.zip` (the zip root must be `codexqa-testdata-generator/SKILL.md`). The result should be `~/.cursor/skills/codexqa-testdata-generator/SKILL.md`.
If the zip was created by compressing the source directory directly, keep the `codexqa-testdata-generator` folder name and remove `__MACOSX`, `dist/`, and `testdata/`.
...
| Symptom | What to do |
|---|---|
| Cannot find `catalog` | Confirm the unzipped directory is named `codexqa-testdata-generator` and `slots/catalog/SKILL.md` exists; invoke scripts with `node "$SKILL_DIR/scripts/<name>.ts"` |
