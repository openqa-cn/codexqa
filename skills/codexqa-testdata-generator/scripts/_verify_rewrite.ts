#!/usr/bin/env node
/**
 * Durable assertion harness for the TypeScript rewrite of codexqa-testdata-generator.
...
function verifyPackSkills(): void {
  ...
  const skillZip = join(packOut, "codexqa-testdata-generator.zip");
  const pluginZip = join(packOut, "codexqa-testdata-generator-claude-plugin.zip");
  ...
  if (existsSync(skillZip)) {
    ...
    record("P05", "skill zip names", "root folder codexqa-testdata-generator/", names.some((n) => n.startsWith("codexqa-testdata-generator/")) ? "ok" : names.slice(0, 5).join(","), names.some((n) => n.startsWith("codexqa-testdata-generator/")));
  ...
  if (existsSync(pluginZip)) {
    ...
    record("P13", "plugin zip", "contains skills/codexqa-testdata-generator/SKILL.md", names.some((n) => n.includes("skills/codexqa-testdata-generator/SKILL.md")) ? "ok" : "missing", names.some((n) => n.includes("skills/codexqa-testdata-generator/SKILL.md")));
...
function printReport(): number {
  ...
  process.stdout.write("\n=== codexqa-testdata-generator rewrite assertion report ===\n");
