#!/usr/bin/env node
/** Create a limited catalog product. */
...
export async function main(params: Record<string, unknown>): Promise<{
  ...
  const body = {
    ...
    kind: "limited",
    validHours: Math.trunc(Number(params.validHours || 4)),
  };
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1]?.endsWith("create_limited_product.ts")) {
  await runMain(main, process.argv, {
    description: "Create a limited catalog product.",
    exampleJson: '{"name":"demo-limited","city":"demo-city","validHours":4}',
  });
}
