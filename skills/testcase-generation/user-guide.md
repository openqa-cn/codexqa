# testcase-generation user guide
...
`{run_dir}` is created or reused by this Skill: your specified directory → existing `userConfig.runDir` → default `$HOME/testdata-generation/runs/{runid}`. `runid` is this task id. Original internal subdirectories (test plan, test cases, and so on) still live under that `run_dir`. Artifacts are not written outside it.
...
Path: `{run_dir}/testcase/testdocs/userConfig.json`. Written automatically on first init; not a remote-space binding wizard. `runDir` must be an expanded absolute path; when no directory is specified, default to `$HOME/testdata-generation/runs/{runid}`.
...
```json
{
  ...
  "runDir": "/Users/name/testdata-generation/runs/20260820-143000",
  "knowledgeSource": "",
  "knowledgeGitUrl": "",
  "knowledgeGitRef": ""
}
...
| `runid` / `runDir` / `initTime` | This task id, run root (default `$HOME/testdata-generation/runs/{runid}`), and init time |
