...
| Term | Meaning |
|------|------|
| **run_dir** | Run root directory, specified by the user or newly created by this Skill; default `$HOME/testdata-generation/runs/{runid}`, where `{runid}` is the task id |
...
if [ -n "${USER_RUN_DIR}" ]; then
  ...
  rundir="$HOME/testdata-generation/runs/${runid}"
fi
...
```json
{
  ...
  "runDir": "<run root directory, e.g. /Users/name/testdata-generation/runs/20260820-143000>",
...
**runDir determination rule**: user-specified path → `userConfig.runDir` already restored in the conversation → default `$HOME/testdata-generation/runs/{runid}`. Both first write and missing-field backfill use that default's expanded absolute path. After writing, later stages only read and reuse from the JSON.

| Field | Type | Default | Description |
|------|------|--------|------|
| `runid` | string | auto-generated | This task id / unique run identifier |
| `runDir` | string | `$HOME/testdata-generation/runs/{runid}` | Run root directory; same as the default working directory; must write the expanded absolute path |
...
| Purpose | Path |
|------|------|
| Run directory (run_dir) | `$HOME/testdata-generation/runs/{runid}` (user-specified or default newly created) |
