...
`codexqa-code-reviewer` is **graph-evidence** code review: collect a CodexQA evidence pack (call chains, blast radius, test edges, dimension signals), have the host agent evaluate registry dimensions, then run an **Agent LLM judgment** pass (host embedded model) that **dedupes/merges** against heuristic findings via `merge-llm-findings.py`, write `review-conclusion.json`, and render bilingual `REVIEW-REPORT.html`. It applies to any language / polyglot monorepo and uses **only** the public **CodexQA CLI** (`@openqa-cn/codexqa`) as the primary analysis backend (no homemade call graph).

It is **not** [`codexqa-defect-analyzer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-defect-analyzer/README.md) (SAST + Agent LLM Detection → `report_scan.*`), **not** [`codexqa-code-analyzer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-code-analyzer/README.md) alone (symbol-graph Q&A without the review HTML pipeline), and **not** [`codexqa-rootcause-analyzer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-rootcause-analyzer/README.md) (exception RCA). Use this skill when the goal is a **CodexQA-backed review report**.
...
|---|---|
| `npx skills add openqa-cn/codexqa --skill codexqa-code-reviewer` | Installs the skill into a coding-agent skills directory |
| `bash scripts/collect-pr-evidence.sh …` | Builds the evidence pack (this is **not** an `npx` binary) |

There is no `npx codexqa-code-reviewer` / package `bin`. After install, point `$SKILL_DIR` at the installed folder and run the scripts under `$SKILL_DIR/scripts/`.
