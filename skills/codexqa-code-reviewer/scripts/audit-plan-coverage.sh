...
if [[ -d "$ROOT" && ("$ROOT" == */skills/codexqa-code-reviewer || "$(basename "$ROOT")" == "codexqa-code-reviewer") ]]; then
  row A1 "源码位于 skills/codexqa-code-reviewer/ (name=codexqa-code-reviewer)" I PASS "path exists"
else
  row A1 "源码位于 skills/codexqa-code-reviewer/" I FAIL "unexpected root: $ROOT"
fi

if has '^name: codexqa-code-reviewer' "$ROOT/SKILL.md"; then
  row A0 "SKILL.md name: codexqa-code-reviewer" I PASS "frontmatter name"
else
  row A0 "SKILL.md name: codexqa-code-reviewer" I FAIL "frontmatter name mismatch"
fi
