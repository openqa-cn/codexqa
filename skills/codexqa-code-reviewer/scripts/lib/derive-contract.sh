#!/usr/bin/env bash
# codexqa-code-reviewer: Derive contract signals (local; 0 extra CodexQA).
# Usage: derive-contract.sh --dir <OUT_DIR> [--mode pr|full] [--repo <REPO>]
set -euo pipefail
...
usage() { cat <<'U'
Usage: derive-contract.sh --dir <OUT_DIR> [--mode pr|full] [--repo <REPO>]
Writes <OUT_DIR>/17-contract-signals.json (API/breaking/XSS sinks).
U
}
while [[ $# -gt 0 ]]; do
  case "$1" in
...
OUT="$DIR/17-contract-signals.json"
...
if { [[ -n "${ACR_PY:-}" ]] || command -v ${ACR_PY:-python3} >/dev/null 2>&1; } && [[ -f "$BODY_PY" ]]; then
  "$SCRIPT_DIR/../acr-python" "$BODY_PY" "$REPO" "$files_file" "$TMPD/body.json" "contract"
else
...
jq -n --arg mode "$MODE" --slurpfile body "$TMPD/body.json" --slurpfile iq "$manifest_file" '
  ...
  ($body[0] // {}) as $b
  | $b + {
      kind: "ContractSignals",
      mode: $mode,
      generated_by: "derive-contract.sh",
      schema_version: 1,
