...
12b. **Agent LLM judgment last (order 16):** after heuristic draft findings exist, follow [prompts/llm-judgment-pass.md](llm-judgment-pass.md) and [references/dimensions/llm-judgment.md](../references/dimensions/llm-judgment.md). Use the **host agent embedded model** to review pack-scoped diffs/sources; write `llm-candidates.json` + `baseline-findings.json`; run `scripts/lib/merge-llm-findings.py` → `merged-findings.json` + `22-llm-judgment.json`. Final `p0`/`p1`/`p2` **must** be the merged lists (duplicates dropped or enriched). Include `llm_judgment` in `dimensions_covered`. Never emit a second finding that restates an existing heuristic issue.
...
- **Agent LLM judgment:** always run the host-agent pass + `merge-llm-findings.py` so
  final findings are deduped; never ship duplicate heuristic+LLM cards for one defect.
