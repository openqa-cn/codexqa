...
When `29-judgment-packet.json` exists, do not open the signal files in steps 6–14b2 and do not hand-write dimension essays or Mermaid. `seal-conclusion.py` fills empty dimension sections from the signal files at render. Those steps apply only when the packet is absent.

## Process

1. Read `09-language-profile.json` / `manifest.primary_language` — lock primary language; note `is_polyglot` + secondaries. If Semgrep, Bandit, gosec, gitleaks, osv-scanner, ruff, or eslint is missing, run `scripts/lib/install-sast-tools.sh` before trusting `23-sast-signals.json`. When `29-judgment-packet.json` exists, a dimension with `signals_thin`, `suspect_count` 0, and no report row gets no dimension essay. `plan_required` false means do not write a coverage checklist first. `truncated` true is the whole read.
...
14c. **Agent LLM judgment last (order 16):** follow [prompts/llm-judgment-pass.md](llm-judgment-pass.md) + [references/dimensions/llm-judgment.md](../references/dimensions/llm-judgment.md). When `29-judgment-packet.json` exists, one pass covers the SAST suspect pass, the business-logic pass, the semantic-candidate pass, and the residual read ([prompts/residual-read-pass.md](residual-read-pass.md)); groups in `judgment-work/` run concurrently. When that packet is absent, run those passes as separate prompts and merge via `scripts/lib/merge-llm-findings.py`. Scanner hits do not remove a symbol from that queue. Excluded symbols stay excluded. Do not copy `coverage_closure`, report-row cards, oracle rows, or Mermaid; `seal-conclusion.py` fills the ledger fields, English mirrors, caller chains, and any empty dimension section (`risk_tier`, `design_fit`, `complexity`, `dependencies`, `resilience`, `privacy`, `rollout`, `performance`, `llm_judgment`) at render from `judgment.json` and the signal files. Include `llm_judgment` in `dimensions_covered`.
