...
12. **Test gaps** — production changed symbols with `tested_count==0` and empty tests-reach.
    Record them in `test_gaps` on `review-conclusion.json`. Do not render that list in HTML. Keep a name in `waived_symbols` (still satisfies the gate) when it is a static initializer (`static` / `<clinit>`), a type or constructor, a `getX` / `setX` / `isX` accessor, or a private helper. A call written in a test is not a coverage edge, so a behavior method the nested test calls still stays in `test_gaps.symbols` when `tested_count==0`.
