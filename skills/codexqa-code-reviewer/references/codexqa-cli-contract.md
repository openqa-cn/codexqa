...
**primary** repository analysis backend for the `codexqa-code-reviewer` skill. Collectors
source `scripts/lib/codexqa-preflight.sh`. Manifest stamps `engine: "codexqa"`,
`skill: "codexqa-code-reviewer"`, and `polyglot_mandate: true`. Alternate tools (git diff,
