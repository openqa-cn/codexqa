...
## 这不是 codexqa-testcase-generator
...
`scripts/run_analysis.ts` 和解析/转换脚本只在 Markdown / JSON / CSV / Word 形态之间搬文本。它们不算风险，也不填登记表。分析是宿主 agent 在走 `prompts/codexqa-requirement-analyzer.md`。
