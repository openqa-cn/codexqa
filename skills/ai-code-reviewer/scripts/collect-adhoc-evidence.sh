#!/usr/bin/env bash
# ai-code-reviewer: Collect single-file / small-set adhoc evidence via CodexQA.
...
if [[ -n "$REPO" ]]; then
  ...
  git -C "$MINI" init -q
  git -C "$MINI" config user.email "adhoc@ai-code-reviewer.local"
  git -C "$MINI" config user.name "ai-code-reviewer-adhoc"
  git -C "$MINI" add -A
