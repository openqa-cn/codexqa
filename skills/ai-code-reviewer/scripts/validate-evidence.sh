#!/usr/bin/env bash
# ai-code-reviewer: Validate CodexQA evidence pack before review reasoning.
...
if [[ -f "$DIR/manifest.json" ]]; then
  ...
  if [[ -n "$SKILL_NAME" && "$SKILL_NAME" != "ai-code-reviewer" && "$SKILL_NAME" != "code-review-codexqa" ]]; then
    WARNINGS+=("manifest.skill=$SKILL_NAME (expected ai-code-reviewer); continuing for compatibility")
  fi
fi
