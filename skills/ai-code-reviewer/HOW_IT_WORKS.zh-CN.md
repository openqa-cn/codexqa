...
[`ai-code-reviewer`](README.zh-CN.md) 是宿主 agent 的脚手架：本地仓库（PR/diff、全仓或 adhoc 文件）进去；CodexQA 证据包与双语 `REVIEW-REPORT.html` 出来。本 skill 不内嵌模型，也不 vendoring CodexQA 源码。收集脚本只调用 `codexqa` 二进制；宿主 agent 依据包内产物评估启发式维度，再跑第 16 维 Agent LLM judgment，并用 `merge-llm-findings.py` 做确定性去重。
...
| [`defect-detection`](https://github.com/openqa-cn/codexqa/blob/main/skills/defect-detection/README.zh-CN.md) | SAST + Agent LLM Detection → `report_scan.*`（去重合并） |
