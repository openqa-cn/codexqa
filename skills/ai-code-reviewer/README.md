...
`ai-code-reviewer` is **graph-evidence** code review: collect a CodexQA evidence pack (call chains, blast radius, test edges, dimension signals), then have the host agent write `review-conclusion.json` and render bilingual `REVIEW-REPORT.html`. It applies to any language / polyglot monorepo and uses **only** the public **CodexQA CLI** (`@openqa-cn/codexqa`) as the primary analysis backend.

It is **not** [`defect-detection`](https://github.com/openqa-cn/codexqa/blob/main/skills/defect-detection/README.md) (SAST+agent `report_scan.*`), **not** [`code-analyzer`](https://github.com/openqa-cn/codexqa/blob/main/skills/code-analyzer/README.md) alone (symbol-graph Q&A without the review HTML pipeline), and **not** [`root-cause-diagnosis`](https://github.com/openqa-cn/codexqa/blob/main/skills/root-cause-diagnosis/README.md) (exception RCA). Use this skill when the goal is a **CodexQA-backed review report**.
...
`SKILL.md` is the entry for an AI agent. Happy path: preflight → collect pack → validate → review from artifacts only → write `review-conclusion.json` → render `REVIEW-REPORT.html`.
...
Depends on the closed-source `@openqa-cn/codexqa` CLI; review prose is model-judged; pack gaps weaken findings. For concrete failure cases see [Known limitations](KNOWN_LIMITATIONS.md). For the data flow see [How it works](HOW_IT_WORKS.md).
