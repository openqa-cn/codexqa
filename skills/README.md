...
- [`codexqa-code-reviewer`](codexqa-code-reviewer/): CodexQA graph-evidence pack + heuristic dims + Agent LLM judgment (dedupe) → bilingual `REVIEW-REPORT.html` (PR/diff, full-repo, or adhoc). Not SAST+agent scan reports (`codexqa-defect-analyzer`) and not structure/impact Q&A alone (`codexqa-code-analyzer`). [How it works](codexqa-code-reviewer/HOW_IT_WORKS.md).
...
- [`codexqa-change-analysis`](codexqa-change-analysis/): one-diff change impact — trusted `index --diff-base --full`, a 7-section HTML report (entries, changed methods, test plan, coverage, sensitive paths), and new runnable test files for uncovered points (add only). Not general symbol-graph Q&A (`codexqa-code-analyzer`). [Skill README](codexqa-change-analysis/README.md).
...
- [`codexqa-code-wiki`](codexqa-code-wiki/): local architecture knowledge graph — index a repo, export community digests with `wiki inputs` (no model), and write a DeepWiki-style HTML wiki (sidebar + article + TOC). [Skill README](codexqa-code-wiki/README.md).
- [`codexqa-jev-browser`](codexqa-jev-browser/): Jev browser automation. A page index replaces GUI-model screenshot recognition, Playwright Chromium is the default browser, and generated YAML replays without another decision call. Reports land in `reports/<run-id>/report.html`. [Skill README](codexqa-jev-browser/README.md).
