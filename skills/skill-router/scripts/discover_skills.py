...
SKILL.md with YAML frontmatter. Excludes this skill (skill-router) and
...
def _reexec_if_needed():
    ...
    if os.environ.get(_REEXEC_ENV) == "1":
        sys.stderr.write(
            "skill-router discover_skills.py needs Python %d.%d+; got %d.%d\n"
    ...
    sys.stderr.write(
        "skill-router discover_skills.py needs Python %d.%d+ on PATH\n"
        % (_MIN_PY[0], _MIN_PY[1])
    )
    sys.exit(1)
