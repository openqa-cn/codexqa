...
`codexqa-defect-analyzer` 跑两条检测维度——确定性的 SAST / lint / secrets / SCA，再由**调用本 skill 的宿主 agent 内嵌大模型**做**一轮 Agent LLM 检测**（默认 `--llm-mode agent`，无需 API key）——然后**去重合并**成按 **P0→P3** 排序的 `report_scan.json` / `.md` / `.html`。代码图与调用链对**所有语言**统一走公开的 **CodexQA CLI**（`@openqa-cn/codexqa`）。

**不是** [`codexqa-code-reviewer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-code-reviewer/README.zh-CN.md)（CodexQA 证据包 HTML 评审），**不是** [`codexqa-code-analyzer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-code-analyzer/README.zh-CN.md)（符号图影响面 / 测试缺口），也**不是** [`codexqa-rootcause-analyzer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-rootcause-analyzer/README.zh-CN.md)（异常根因）。目标是代码风险 / 安全 / 逻辑的**扫描报告**时用本 skill。
...
| 命令 | 作用 |
|---|---|
| `npx skills add openqa-cn/codexqa --skill codexqa-defect-analyzer` | 把 skill 装进 Coding Agent 的 skills 目录 |
| `python3 scripts/run_scan.py …` | 跑扫描编排器（**不是** `npx` 二进制） |

没有 `npx codexqa-defect-analyzer` / package `bin`。安装后把 `$SKILL_DIR` 指到已安装目录，再跑 `python3 "$SKILL_DIR/scripts/run_scan.py"`。
