# CodexQA CLI (codexqa-defect-analyzer)

Graph / call-chain / cross-file correlation for **codexqa-defect-analyzer** goes through `scripts/codexqa_client.py` only. Do **not** reimplement symbol graphs, callers/callees, imports, reachability, or RAG from homemade analyzers.
...
## Mock policy (codexqa-defect-analyzer)
...
Unless the user explicitly asks (outside the codexqa-defect-analyzer report path):
