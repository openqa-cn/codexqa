...
[`codexqa-defect-analyzer`](README.md) is scaffolding for a host agent: a diff, repo, upload, or paste goes in; a P0–P3 `report_scan.*` comes out. Two detection dimensions run, then Python **dedupes and merges** them into one report:
...
| Skill | Difference |
|---|---|
| [`codexqa-code-reviewer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-code-reviewer/README.md) | CodexQA evidence pack + heuristic dims + Agent LLM judgment (dedupe) → bilingual `REVIEW-REPORT.html` — no `report_scan` pipeline |
| [`codexqa-code-analyzer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-code-analyzer/README.md) | Structure/impact from the same CodexQA graph — not a finding severity report |
| [`codexqa-rootcause-analyzer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-rootcause-analyzer/README.md) | Exception RCA on top of CLI facts — not incremental/full code-risk scan |
