...
function parseArgs(argv) {
    ...
    for (let i = 1; i < rest.length; i += 1) {
        ...
        if (key === '--body-file')
            ...
            out.vars.baseBranch = value;
        else
            out.vars[key.slice(2)] = value;
    }
    return out;
}
...
function applyAuth(url, headers, auth) {
    ...
    const missing = (label) => {
        if (required)
            throw new AuthMissingError(`auth required but ${label} is empty`);
        return { url, headers };
    };
    if (type === 'bearer') {
        const token = process.env[auth?.tokenEnv || 'CODE_REVIEWER_TOKEN'] || '';
        if (!token)
            return missing(auth?.tokenEnv || 'CODE_REVIEWER_TOKEN');
    ...
    if (type === 'basic') {
        ...
        if (!user || !pass)
            return missing('CODE_REVIEWER_USER / CODE_REVIEWER_PASSWORD');
