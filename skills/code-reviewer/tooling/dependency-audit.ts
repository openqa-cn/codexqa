...
function getChangedFilesFromDiff(): string[] {
...
function extractImportsFromFiles(files: string[]): string[] {
...
async function checkPhantomDependencies(changedFiles: string[]): Promise<CheckResult> {
...
function checkVulnerabilities(): CheckResult {
  ...
  try {
    ...
    const audit = JSON.parse(auditResult) as AuditReport;
    const vuln = audit.metadata?.vulnerabilities || audit.vulnerabilities || {};
    const critical = vuln.critical || 0;
    const high = vuln.high || 0;

    console.log(
      `  🔴 critical: ${critical}  🟠 high: ${high}  🟡 moderate: ${vuln.moderate || 0}  🟢 low: ${vuln.low || 0}\n`,
    );
...
async function main(): Promise<void> {
  ...
  const phantomResult =
    changedFiles.length > 0 ? await checkPhantomDependencies(changedFiles) : { passed: true };
