...
const INTEGRATION_NAMES = ['prMetadata', 'notify', 'telemetry', 'extraKnowledge'] as const;

interface LoadedConfigOut {
  source: string;
  sources: unknown[];
  config: {
    version: number;
    git: { defaultBaseBranch: string; codeBrowseUrlTemplate: string };
    layers: {
      ui: string[];
      store: string[];
      api: string[];
    ...
    integrations: Record<string, { enabled: boolean; url: string }>;
  };
}
...
interface ProgressFile {
  mode: string;
  base: string;
  phases: Array<{ name: string; status: string; findings_file: string | null; error: string | null }>;
...
test('load-config prints built-in defaults when no project file exists', () => {
  ...
  try {
    ...
    const loaded = JSON.parse(result.stdout) as LoadedConfigOut;
...
test('invoke-integration skips a disabled extraKnowledge endpoint', () => {
  ...
  try {
    ...
    assert.equal(result.status, 0, result.stderr);
    const body = lastJsonLine(result.stdout) as SkipOut;
...
test('review-progress state machine: init, phases, counts, note, cleanup', () => {
  ...
  try {
    ...
    let data = JSON.parse(readFileSync(progressPath, 'utf-8')) as ProgressFile;
    ...
    data = JSON.parse(readFileSync(progressPath, 'utf-8')) as ProgressFile;
...
interface SecurityScanOut {
  files: number;
  critical: number;
  high: number;
  medium: number;
  findings: Array<{ id: string; file: string; line: number; rule: string; severity: string }>;
...
test('security-scan partitions languages and ignores JS template ${} plus placeholder secrets', () => {
  ...
  try {
    ...
    writeFileSync(
      join(cwd, 'OrderMapper.xml'),
      '<select id="find">SELECT id FROM orders WHERE id = ${id}</select>\n',
    );
    writeFileSync(
      join(cwd, '.cr-files.txt'),
      ['app.ts', 'secrets.ts', 'OrderMapper.xml'].join('\n'),
    );

    const result = run('security-scan.js', ['--files', '.cr-files.txt'], cwd);
    assert.equal(result.status, 0, result.stderr);
    const body = JSON.parse(result.stdout) as SecurityScanOut;
    ...
    assert.equal(
      body.findings.some((item) => item.file === 'app.ts' && (item.id === 'S9' || item.id === 'S1')),
      false,
      result.stdout,
    );
...
test('security-scan skips macOS zip junk under __MACOSX', () => {
  ...
  try {
    ...
    writeFileSync(
      join(cwd, '__MACOSX', 'secrets.ts'),
      "const apiKey = 'sk-live-abcdefghijklmnopqrstuvwxyz';\n",
    );
    writeFileSync(join(cwd, '.DS_Store'), 'junk');
    writeFileSync(
      join(cwd, '.cr-files.txt'),
      ['__MACOSX/secrets.ts', '.DS_Store'].join('\n'),
    );
    const result = run('security-scan.js', ['--files', '.cr-files.txt'], cwd);
    assert.equal(result.status, 0, result.stderr);
    const body = JSON.parse(result.stdout) as SecurityScanOut;
