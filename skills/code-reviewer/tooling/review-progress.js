...
function cmdPhaseStart(args) {
    const name = args[0];
    if (!name)
        throw new Error('Usage: review-progress phase-start <phase_name>');
...
function cmdPhaseDone(args) {
    const name = args[0];
    if (!name)
        throw new Error('Usage: review-progress phase-done <phase_name> [--findings <file>]');
    ...
    if (findingsFile)
        phase.findings_file = findingsFile;
    data.last_checkpoint = nowISO();
    writeProgress(data);
...
function cmdPhaseFail(args) {
    const name = args[0];
    if (!name)
        throw new Error('Usage: review-progress phase-fail <phase_name> [--reason <text>]');
...
function cmdUpdateCounts(args) {
    ...
    const data = readProgress();
    if (!Number.isNaN(p0))
        data.findings_so_far.p0 = p0;
    if (!Number.isNaN(p1))
        data.findings_so_far.p1 = p1;
    if (!Number.isNaN(p2))
        data.findings_so_far.p2 = p2;
    data.last_checkpoint = nowISO();
    writeProgress(data);
    console.log(`📊 Updated counts: P0=${data.findings_so_far.p0} P1=${data.findings_so_far.p1} P2=${data.findings_so_far.p2}`);
}
function cmdNote(args) {
    const text = args.join(' ').trim();
    if (!text)
        throw new Error('Usage: review-progress note <text>');
    const data = readProgress();
...
function cmdStat() {
    ...
    console.log(`Diff          : ${data.diff_stats.files_changed} files / ${data.diff_stats.lines_changed} lines / ${data.diff_stats.size_kb} KB`);
    console.log(`Findings so far: P0=${data.findings_so_far.p0}  P1=${data.findings_so_far.p1}  P2=${data.findings_so_far.p2}`);
