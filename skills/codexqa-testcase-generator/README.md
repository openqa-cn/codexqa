...
It is **not** [`codexqa-requirement-analyzer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-requirement-analyzer/README.md) (gap/conflict register), **not** [`codexqa-testdata-generator`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-testdata-generator/README.md) (backend data construction / write-back), and **not** a code-risk scanner (`codexqa-defect-analyzer` / `codexqa-code-reviewer`).
...
Default run directory: `$HOME/codexqa-testdata-generator/runs/{runid}`. You may also specify a local directory. Original internal subdirectories still live under that `run_dir`.
...
- Does not construct live backend test data (that is `codexqa-testdata-generator`)
