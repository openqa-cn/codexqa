...
**不是** [`codexqa-requirement-analyzer`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-requirement-analyzer/README.zh-CN.md)（缺口 / 冲突登记表），**不是** [`codexqa-testdata-generator`](https://github.com/openqa-cn/codexqa/blob/main/skills/codexqa-testdata-generator/README.zh-CN.md)（后端造数 / 回写），也**不是**代码风险扫描（`codexqa-defect-analyzer` / `codexqa-code-reviewer`）。
...
默认运行目录：`$HOME/codexqa-testdata-generator/runs/{runid}`。也可指定本地目录。内部子目录仍在该 `run_dir` 下。
...
- 不从需求正文自动 clone Git
- 提测后增量在本 Skill 内完成
- 不构造真实后端测试数据（那是 `codexqa-testdata-generator`）
