...
Do not **guess** `run_dir` from the home directory or a fixed prefix (must read the already-written `userConfig.runDir`). `userConfig.runDir` defaults to `$HOME/codexqa-testdata-generator/runs/{runid}`; see [run-workspace.md](../run-workspace.md). Do not write artifacts outside `{run_dir}`.
