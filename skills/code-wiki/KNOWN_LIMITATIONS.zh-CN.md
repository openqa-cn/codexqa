...
`code-wiki` 画社区、真实依赖和上手路径。它不跑不带 `--no-llm` 的 `codexqa wiki`，不跑 `wiki embed` 或 `query wiki`，不审变更，不打 P0 / P1 / P2。影响面和测试缺口用 `code-analyzer`，需求类缺陷用 `defect-detection`，实现审查用 `code-reviewer`。
