...
**不是** [`code-analyzer`](../code-analyzer/README.zh-CN.md)（符号图影响面 / 调用方 / 测试缺口），**不是** [`defect-detection`](../defect-detection/README.zh-CN.md)（带写回门禁的需求类缺陷），也**不是** [`code-reviewer`](../code-reviewer/README.zh-CN.md)（P0/P1/P2 playbook 审查）。那些负责结构、需求或审查发现；输入是异常、目标是根因分析时用本 skill。
