...
function row(partial: Record<string, any> = {}) {
  return {
    parentBatchId: 1,
    className: "com/example/SecurityCodeType",
...
describe("writeback last-write-wins protect", () => {
  ...
  it("treats short className and FQCN as the same write-back key", () => {
    assert.equal(
      writeback_identity_match(
        row({ className: "com/example/SecurityCodeType" }),
        row({ className: "SecurityCodeType" }),
      ),
      true,
    );
    assert.equal(
      writeback_identity_match(
        row({ className: "com/example/SecurityCodeType" }),
        row({ className: "com/other/SecurityCodeType" }),
      ),
      false,
    );
  });
  ...
  it("protects FQCN defect from a short-name no-defect overwrite", () => {
    ...
    store.record_process_writeback(row({
      className: "com/sankuai/meituan/banma/staff/IntelligenceAdaptor",
      methodName: "doQueryIntelligence",
      bugStatus: 6,
    }));
    const rec = store.record_process_writeback(row({
      className: "IntelligenceAdaptor",
      methodName: "doQueryIntelligence",
      bugStatus: 2,
    }));
    ...
    assert.equal(kept.bugStatus, 6);
    assert.equal(kept.methodName, "doQueryIntelligence");
  });
