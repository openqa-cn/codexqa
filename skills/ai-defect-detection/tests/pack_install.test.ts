...
describe("pack + install", () => {
  it("frontmatter is agent-skills compatible", () => {
    ...
    assert.match(text, /^name: ai-defect-detection$/m);
    const desc = text.match(/^description:\s*(.+)$/m);
    assert.ok(desc);
    assert.ok(desc[1].trim().length <= 160);
  ...
  it("zip install then run from other cwd", () => {
    ...
    const zipPath = join(tmp, "ai-defect-detection.zip");
    assert.ok(existsSync(zipPath));
    const listing = spawnSync("unzip", ["-l", zipPath], { encoding: "utf8" });
    assert.match(listing.stdout, /ai-defect-detection\/SKILL.md/);
    assert.match(listing.stdout, /ai-defect-detection\/open_detect.ts/);
    assert.doesNotMatch(listing.stdout, /\.jar/);
    assert.doesNotMatch(listing.stdout, /\/tests\//);
    assert.doesNotMatch(listing.stdout, /open_detect\.py/);
