...
import { NO_DEFECT_CONTENT, _cli_error, _cli_result, _cli_success, _require_save } from "./open_cli_common.ts";
import {
  ContentStore,
...
import { prepare_rules } from "./open_rules.ts";
import { validate_update_process, validate_update_process_all, set_valid_tag_ids } from "./open_validate.ts";
...
function _reconcile_file_codes(
  store: ContentStore,
  class_name: any,
  method_name: any,
  strategy_code: any,
  ...
  const is_frontend = Boolean(ctx.isFrontend);
  ...
  if (!(auth_fp && auth_git)) {
    ...
    return [file_codes, notes, server_class_name, is_frontend];
  }

  if (!file_codes || !file_codes.length) {
    notes.push("Incoming fileCodes was empty; filled from the local authoritative mapping.");
    return [[ctx.fileCodesItem], notes, server_class_name, is_frontend];
  }

  const corrected: any[] = [];
  for (let fc of file_codes) {
  ...
  return [corrected, notes, server_class_name, is_frontend];
}
...
export function cmd_update_process(args: Args): void {
  ...
  if (args.task_id != null) {
    ...
    let _notes: string[];
    let _is_frontend: boolean;
    [file_codes, _notes, _server_class_name, _is_frontend] = _reconcile_file_codes(
    ...
    if (_is_frontend) {
      ...
      console.error("[update-process] 🔧 Frontend files will write back at file-level process and join rank aggregation");
    }
  }
...
export function cmd_finalize_rank(args: Args): void {
  ...
  if (missing_fields.length) {
    ...
    console.error("[finalize-rank] Fill the fields from the SKILL-WRITEBACK.md template and retry.");
  ...
  if (task_id) {
    ...
    _git_file_path = _ctx.isFrontend ? _ctx.filePath : null;
    if (_git_file_path) {
      console.error(`[finalize-rank] 🔧 Frontend files will use gitFilePath=${JSON.stringify(_git_file_path)} for file-level rank aggregation`);
    }
  }
...
export function cmd_gen_writeback_template(args: Args): void {
  ...
  const git = "git@github.com:<org>/<repo>.git";
  const branch = "feature/xxx";
  const commit = "<commitId>";
  let file_path = class_name ? class_name.replace(/\//g, "/") + ".java" : "src/main/java/com/example/ExampleService.java";
  file_path = file_path.startsWith("src/") ? file_path : "src/main/java/" + file_path;
  const batch_id = 0;
  ...
  const template: Record<string, any> = {
    ...
    processId: "<fill: processId from pending>",
    className: class_name,
    methodName: method,
    bugStatus: bug_status,
    strategyCode: strategy,
    ...
    thinking: `Review ${method} lines X-Y: <which logic was reviewed, which Risks were considered, cite concrete Lines/variable names>.` +
      (has_defect ? "Found a defect: ..." : "Parameter checks are complete, exception handling is reasonable, no logic defect."),
    content: !has_defect ? "No defect in this code" : (
      ...
      "Reproduction path: <Trigger conditions → which code is hit → what result>\n\n" +
  ...
  if ([6, 11].includes(strategy)) {
    ...
    if (has_defect) {
      template._hint_strategy11_defect = "When bugStatus=6/7, content must also include a test-case citation: Test case ID: [<id>](<link>)";
    }
  } else if (strategy === 8) {
...
export function cmd_batch_update_process(args: Args): void {
  ...
  if (args.task_id != null) {
    ...
    for (const it of items) {
      ...
      const [fixed_fc, notes, server_class_name, is_frontend] = _reconcile_file_codes(
      ...
      if (is_frontend) {
        ...
        console.error(`[batch-update-process] 🔧 Frontend files will write back at file-level process: ${it.className} → ${git_file_path}`);
      }
    ...
    if ([6, 7].includes(bug_st)) {
      ...
      if (target_mn && first_line) {
        ...
        for (const fc of it.fileCodes || []) {
          ...
          try {
            if (_is_dir("/tmp/defect-detection")) {
              for (const local_candidate of readdirSync("/tmp/defect-detection/")) {
                const local_dir = `/tmp/defect-detection/${local_candidate}`;
                const local_file = join(local_dir, file_path);
                if (_is_file(local_file)) {
                  try {
          ...
          if (commit_id) {
            const repo_name = basename(git_url.replace(/\/+$/, "")).replace(/\.git$/, "");
            const tmp_clone = `/tmp/_valid_${repo_name}`;
  ...
  if (pre_errors.length) {
    console.log(JSON.stringify({
      code: -2,
      msg: `pre-validation blocked: ${pre_errors.length}/${items.length} item(s) failed; nothing was submitted`,
      errors: pre_errors,
    }, null, 2));
    process.exit(1);
  }
...
export function cmd_read_method_code(args: Args): void {
  ...
  let candidates = _glob_star(local_dir, `**/${class_path}.*`);

  const source_exts = new Set([".java", ".kt", ".scala", ".py", ".js", ".ts", ".tsx", ".jsx", ".go"]);
  let source_files = candidates.filter((f) => source_exts.has(extname(f).toLowerCase()));

  if (!source_files.length) {
    const simple_name = class_path.split("/").pop()!;
    const candidates2 = _glob_star(local_dir, `**/${simple_name}.*`);
    source_files = candidates2.filter((f) => source_exts.has(extname(f).toLowerCase()));
  }
  ...
  if (method_name && target_file.endsWith(".java")) {
    const lines = content.split("\n");
    const method_pattern = new RegExp(
      `(?:public|private|protected|static|\\s)*\\s+\\w+[\\w<>\\[\\],\\s]*\\s+${method_name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*\\(`,
    );
    let brace_count = 0;
    let in_method = false;
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      if (!in_method && method_pattern.test(line)) {
        in_method = true;
      ...
      if (in_method) {
        ...
        brace_count += (line.match(/\{/g) || []).length - (line.match(/\}/g) || []).length;
        if (brace_count <= 0 && method_lines.join("").includes("{")) {
          method_end_line = i + 1;
          break;
        }
      }
    }
    if (method_lines.length) method_code = method_lines.join("\n");
  }
...
export function classify_trivial_method(method: Record<string, any>): Record<string, any> {
  ...
  if (body_lines != null && body_lines === 0) return _mark("EMPTY_METHOD", "T0");

  if ((method_name.startsWith("get") || method_name.startsWith("is"))
      && !params.includes(",")
      && body_lines != null && body_lines <= 3) {
    return _mark("GETTER", "T0");
  }

  if (method_name.startsWith("set") && method_name.length > 3
      && body_lines != null && body_lines <= 3) {
    return _mark("SETTER", "T0");
  }

  if (method_name === "toString" || method_name === "hashCode") return _mark("TO_STRING", "T0");
  if (method_name === "equals" && params === "Object") return _mark("EQUALS", "T0");

  if ((method_name.startsWith("with") || ["builder", "toBuilder", "newBuilder"].includes(method_name))
      && body_lines != null && body_lines <= 3) {
    return _mark("BUILDER_METHOD", "T0");
  }
...
export function stamp_trivial_on_plan(task_id: any, results: any[]): number {
  ...
  for (const item of plan) {
    ...
    const hit = results.find((r: any) => {
      const rcn = r.className || "";
      const rsimple = rcn.includes("/") ? rcn.split("/").pop()! : String(rcn).split(".").pop()!;
      return r.methodName === mn && (rcn === cn || rsimple === simple || rsimple === cn || simple === rcn);
    });
    if (!hit) continue;
...
export function submit_trivial_writebacks(task_id: any, results: any[]): Record<string, any> {
  ...
  for (const r of trivial) {
    ...
    const simple = cn.includes("/") ? cn.split("/").pop()! : String(cn).split(".").pop()!;
    const plan_item = plan.find((p: any) => {
      const pcn = p.className || "";
      const psimple = pcn.includes("/") ? pcn.split("/").pop()! : String(pcn).split(".").pop()!;
      return p.methodName === mn && (pcn === cn || psimple === simple || psimple === cn);
    });
...
export function cmd_run_ast_scan(args: Args): void {
  ...
  if (!ast_rules.length) {
    console.error(
      "[run-ast-scan] ❌ No category=1 AST rules found." +
        "A Java service always has AST rules; an empty list means get-rules failed or data is wrong, " +
  ...
  if (project_lang && project_lang.toLowerCase() !== "java") {
    console.error(`[run-ast-scan] ⚠️  Project language is ${project_lang}; AST rules apply only to Java services, skipping AST scan.`);
    console.log(JSON.stringify({
      ruleCount: ast_rules.length, totalRuleCount: ast_rules.length,
      skippedRuleCount: ast_rules.length, projectLanguage: project_lang,
      scannedFiles: 0, totalFindings: 0,
      findings: [], skipped: true,
      skipReason: `AST rules only apply to Java; current language: ${project_lang}`,
    }, null, 2));
    return;
  }

  if (target_files) {
    if (project_lang && _LANG_TO_EXTENSIONS[project_lang]) {
      const valid_exts = _LANG_TO_EXTENSIONS[project_lang];
      if (valid_exts.size) {
        const lang_files = target_files.filter((f) => valid_exts.has(extname(f).toLowerCase()));
        if (!lang_files.length) {
          console.error(`[run-ast-scan] ⚠️  No ${project_lang} files in the change set; skip AST scan`);
          console.log(JSON.stringify({
            ruleCount: ast_rules.length, totalRuleCount: ast_rules.length,
            skippedRuleCount: 0, projectLanguage: project_lang,
            scannedFiles: 0, totalFindings: 0,
            findings: [], errors: [`No ${project_lang} files in the change set`],
          }, null, 2));
          return;
        }
  ...
  const findings = annotate_findings_in_diff(result.findings || [], scope.annotateDiffFiles);
...
export function cmd_update_rank_content(args: Args): void {
  ...
  if (common_missing.length) {
    ...
    console.error("[update-rank-content] Fill the missing fields using the SKILL-WRITEBACK.md template.");
