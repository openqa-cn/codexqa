...
.../scripts/ai-defect-detection/
# After install into a tool it must be named ai-defect-detection (matches SKILL.md name)
~/.cursor/skills/ai-defect-detection/
```

**Distribution format is zip / directory. Do not pack a jar.** Cursor, OpenClaw, Claude Code, and similar hosts load Agent Skills: after unzip, the top level must be `ai-defect-detection/SKILL.md`.

```bash
cd "$SKILL_DIR"
bash pack-skill.sh            # produces ai-defect-detection.zip
# Cursor
unzip ai-defect-detection.zip -d ~/.cursor/skills/
# OpenClaw
unzip ai-defect-detection.zip -d ~/.openclaw/skills/
# Or inside a project
unzip ai-defect-detection.zip -d .cursor/skills/
```
...
```bash
export SKILL_SCRIPT="$SKILL_DIR/open_detect.ts"   # after install this is {baseDir}/open_detect.ts
node "$SKILL_SCRIPT" --help
```

Agent execution details are still authoritative in `SKILL.md` / `SKILL-PHASE1.md` / `SKILL-PHASE2.md` / `SKILL-WRITEBACK.md` / `SKILL-POST.md`. This manual is "how a person uses it and which commands to type".
...
| Item | Requirement |
|---|---|
| Python | 3.10+ (stdlib is enough; local mode has no third-party packages) |
...
Chat writes (`add-test-case` / `add-document`) go only into `data/<taskId>/` and do not change `enterprise/`.
...
node "$SKILL_SCRIPT" phase1-fetch-all \
  --task-id "$TASK_ID" --git "$GIT_URL" --user-id "$USER_ID" --plan-id 1001
# or split:
node "$SKILL_SCRIPT" get-tag-list
...
`open_validate.ts` is the format authority: when it reports a format error, fix per it. Do not tweak wording to cheat the quality gate.
...
node "$SKILL_SCRIPT" complete-task --task-id "$TASK_ID" --batch-ids "$BATCH_ID"
# Or finish Phase 3 in one command:
node "$SKILL_SCRIPT" finalize-all --task-id "$TASK_ID"
```
...
Local files ↔ HTTP slots are one-to-one and can be switched per system. Contract: `docs/enterprise-http-api.md`. The detection platform itself: `docs/platform-api.md`.

| System | Local | Default HTTP |
|---|---|---|
| Detection tasks / process / rank | `data/platform/` | `/v1/tasks` etc. in `docs/platform-api.md` |
...
Path variables: `{plan_id}` `{id}` `{case_id}` `{issue_id}` `{doc_id}`. Slot table: `providers/http_slots.ts`. Systems not connected yet stay `kind: local`.
...
| Write-back format failed | Auto-fix deterministic items per `open_validate.ts`; quality issues need re-analysis |
...
| Tags / rules empty | Local is seeded from `providers/platform/seed_catalog.ts`; HTTP platforms use `/v1/tags` and `/v1/rules` |
...
| `SKILL-PHASE1.md` / `SKILL-PHASE2.md` / `SKILL-WRITEBACK.md` / `SKILL-POST.md` | Per-phase Hard rule |
| `docs/adapters.md` | How to change config when switching company systems |
| `docs/platform-api.md` | Detection-platform HTTP |
| `docs/enterprise-http-api.md` | Plan / case / doc / ticket / trace slots |
