...
export const ANALYZE_TIMEOUT = 90;
export const VERSION_TIMEOUT = 20;

export function _which(cmd: string): string | null {
  if (!cmd) return null;
...
export function graph_exists(local_dir: string): boolean {
  if (!local_dir) return false;
  for (const rel of [".gitnexus", "gitnexus.db", ".gitnexus/graph.json", ".gitnexus/index.json"]) {
    const p = join(local_dir, rel);
    if (existsSync(p)) return true;
  }
  return existsSync(_analyze_stamp_path(local_dir));
}
...
export function ensure(
  ...
  out.nextActions = [
    ...
    "Agent may retry once with: node open_detect.ts ensure-gitnexus --force-install",
