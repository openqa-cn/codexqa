...
> **Validation authority**: `open_validate.ts` is the only programmatic implementation of the rules. This file is for understanding; if they conflict, the code validation wins.
>
...
> **Dedup note**: `open_validate.ts` already hard-intercepts validation rules 0–22 (including Affected business / Reproduction path / plain-text format / Problem tags / exclusion_rule_filter / register-code-read / register-repo-clone, etc.). Submissions that fail are blocked automatically with clear fix guidance. **Do not repeat that list here.** Full intercept items: `references/rules/validation-rules.md`.
>
> The table below **only lists document-level soft constraints that open_validate.ts does not intercept** (mainly frontend/client-stack wording). The Agent must check them itself.
...
| content also outputs it | When bugStatus=6/7 and in doubt, content must also include `[Confidence:LOW]` and a `[Pending confirmation]` section (see format example below) | — |
...
> **Why you still write back bugStatus=6/7 when in doubt**: in doubt ≠ false positive. If the code really has a triggerable defect path, even if final confirmation depends on business config or usage, write it back so the user knows and can decide. Open questions are told to the user via `Pending confirmation`; the user decides whether a fix is needed.
...
**Unmet soft constraints are not programmatically blocked, but they lower report quality. Check them yourself before submit. All other field/format issues are intercepted by open_validate.ts; you do not need to "submit first, then fix from errors".**

---

## Field constraints

> **Authoritative full definition**: field constraints and the content format below are authoritative in this file; `references/writeback/writeback-quickref.md` is a compact mirror (for a quick pre-write-back check). If they disagree, this file wins; ultimately `open_validate.ts` programmatic validation wins.
...
When the defect is actually in a called method (see `SKILL-PHASE2.md` STEP C.7), the independently written callee process must include source from both the callee and the caller:
...
```bash
python open_detect.ts resolve-writeback-context \
  --task-id <TASK_ID> \
...
- `filePath`: from the plan item (the real path registered by `set-plan-file-path`); no more concatenation
...
```bash
python open_detect.ts update-process --task-id <TASK_ID> --process-file process.json
python open_detect.ts batch-update-process --task-id <TASK_ID> --process-file batch.json
```
