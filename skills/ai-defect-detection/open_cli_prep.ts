...
import { _cli_error, _cli_result, _cli_success, _inject_strategy_codes, _require_save } from "./open_cli_common.ts";
...
import { ingest_resolved_materials, resolve_plan_info } from "./open_ingest.ts";
import { ensure as ensure_gitnexus, run_cli as gitnexus_run_cli } from "./open_gitnexus.ts";
import { run_prep_and_plan } from "./open_cli_auto.ts";
...
function stamp_service_clone_meta(
  ...
  try {
    ...
    const svc = store.get_service_by_git_url(git_url);
    if (!svc) return;
    ...
    console.error(`[clone] ⚠️  failed to stamp service commitId/localDir: ${exc?.message || exc}`);
  }
}
...
export function cmd_get_plan_info(args: Args): void {
  ...
  if (!services.length) {
    ...
    const _terminal_languages = new Set(["javascript", "typescript"]);
    if (services.some((svc: any) => _terminal_languages.has(String(svc.language || "").toLowerCase()))) {
      is_client_app = true;
    }
  }
  ...
  if (is_client_app && !test_case_ids.length && !issue_list.length) {
    ...
    console.error(`  node open_detect.ts fetch-test-cases --task-id <TASK_ID> --plan-id ${args.plan_id}`);
...
export function cmd_get_pending(args: Args): void {
  ...
  const task_id = data.length ? data[0].taskId : null;
  ...
  if (git_url && !user_id) {
    ...
    console.error(
      "[astAndCustomRules] ⚠️  only --user-id was passed (no --git). " +
        ...
        "  node open_detect.ts get-rules --git <ssh://git@...> --user-id <your_user_id>",
    );
  }
...
function _tag_cache_path(): string {
  return join(__dirname, "data", "_tag_cache.json");
}
...
export function cmd_clone_and_diff(args: Args): void {
  ...
  const language = (args.language || "").trim().toLowerCase() || null;
  const is_client_repo = language === "javascript" || language === "typescript";

  const repo_key = `${git_url.trim()}@${branch.trim()}`;
  const repo_hash = createHash("md5").update(repo_key, "utf8").digest("hex").slice(0, 12);
  const local_dir = `/tmp/defect-detection/${repo_hash}`;
  ...
  stamp_service_clone_meta(task_id, git_url, local_dir, branch, commit_id);
...
function _cmd_gitnexus_tool(args: Args, subcommand: string, extra: string[]): void {
  ...
  if (!extra.includes("--repo")) {
    const name = local_dir.replace(/\/+$/, "").split("/").pop()!;
    extra = ["--repo", name, ...extra];
  }
...
export function cmd_check_phase2_readiness(args: Args): void {
  ...
  for (const svc of services) {
    ...
    const diff_files = svc.diffFiles || global_diff_files;
    if (!diff_files || !diff_files.length) {
      issues.push(`⚠️ service ${service_key} has empty diffFiles; confirm git diff has been run`);
    }
  }

  const meta_info = meta.meta || {};
  if (!store.get_meta_field("tagIds")) {
    issues.push("❌ tagIds is empty: run get-tag-list first");
  }
  if (!store.get_meta_field("rules")) {
    issues.push("⚠️ rules not recorded (non-blocking, but fetch rules first if possible)");
  }
  ...
  const client_languages = new Set(["javascript", "js", "typescript", "ts", "jsx", "tsx", "kotlin", "swift"]);
  const is_client_plan = services.some((svc: any) =>
    svc.serviceKey === "client_plan" || client_languages.has(String(svc.language || "").toLowerCase()),
  );
  if (is_client_plan && test_cases.length) {
...
function _run_subprocess(cmd: string[], timeout = 30): [number, string, string] {
  try {
    ...
    if (r.error) {
      ...
      if (err.code === "ENOENT") return [-2, "", `command not found: ${cmd[0]}`];
      if (err.code === "ETIMEDOUT" || r.signal) return [-1, "", `timeout after ${timeout}s`];
      return [-3, "", String(err.message || err)];
    }
    return [r.status ?? 1, (r.stdout || "").trim(), (r.stderr || "").trim()];
  } catch (e: any) {
    return [-3, "", String(e)];
  }
}
