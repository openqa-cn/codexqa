...
`bugStatus` meanings are globally consistent. See `SKILL-WRITEBACK.md` → bugStatus semantics (unified value domain, globally consistent).
...
node "$SKILL_SCRIPT" update-process \
  ...
  --content "<fill from the full SKILL-WRITEBACK.md template>" \
  --tag-id <tagId> \
  --file-codes '[...]' \
  --process-steps '[...]'
...
Pull the latest code one by one to judge whether it is already fixed. If fixed, call `update-recheck-info` to update the status.
...
node "$SKILL_SCRIPT" update-process \
  ...
  --content "<full SKILL-WRITEBACK.md template>" \
