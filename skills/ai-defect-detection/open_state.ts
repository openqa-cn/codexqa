...
import { content_base_dir } from "./open_store.ts";
...
export function _save_local_state(task_id: number, state: Record<string, any>): void {
  const path = _get_local_state_path(task_id);
 ...
 * Accepts slash↔dot FQCN and short name ↔ full path (`SecurityCodeBiz` ↔ `com/.../SecurityCodeBiz`).
...
export function get_repo_clone_dir(
  batch_id: number,
  git_url: string,
  task_id: number | null = null,
): string | null {
  ...
  if (cached) {
    const local_dir = cached.localDir || "";
    if (_is_dir(local_dir)) return local_dir;
  }
  ...
  if (task_id != null) {
    ...
    const batch_key = String(batch_id);
    if (batch_key in registry && git_url in registry[batch_key]) {
      const local_dir = registry[batch_key][git_url].localDir || "";
      if (_is_dir(local_dir)) return local_dir;
    }
  }
