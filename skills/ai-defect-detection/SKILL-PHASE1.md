...
7. **Load each Phase doc once**: do not re-read this file or `SKILL-PHASE2.md` / `SKILL-POST.md` after the first load unless a gate cites an unread section. After filter / `prep-and-plan`, enter Phase 2 without reloading `SKILL-PHASE2.md`.
8. **Do not browse the PR page** for source/target branch. Use gitUrl+branch from the user / plan / onesId. `clone-and-diff` computes the merge-base. Confirm git access with `clone-and-diff`, not a browser.
...
|---|---|---|
| `$SKILL_SCRIPT` | 1.1 | Absolute path to open_detect.ts (`node "$SKILL_SCRIPT"`) |
...
Locate `open_detect.ts` (prefer a path relative to the skill directory → common install directories → find as fallback). Invoke with `node "$SKILL_SCRIPT"`.
...
| Code PR URL (`.../pr/<id>/...`) plus onesId / branch in the user message | Extract onesId + source branch from the message; `submit-git` / `clone-and-diff`. **Do not open the PR in a browser** |
...
**Extract**: `$TASK_ID`. Then run `init-content` to initialize content.json. If `init-content` / `phase1-init` returns `remappedFrom`, overwrite `$TASK_ID` with the new `taskId` and leave the old directory untouched.
...
```bash
node "$SKILL_SCRIPT" phase1-fetch-all --task-id $TASK_ID --git "$GIT_URL" --user-id "$USER_ID" --plan-id $TEST_PLAN_ID
```
...
node "$SKILL_SCRIPT" get-tag-list --task-id $TASK_ID
node "$SKILL_SCRIPT" get-rules --git "$GIT_URL" --user-id "$USER_ID"
node "$SKILL_SCRIPT" get-confirmed-defect-history --task-id $TASK_ID
...
| Historically confirmed defects | get-confirmed-defect-history | Optional |
...
**Code-read gate**: register-code-read is a **mandatory action, not an optional optimization**. The unified rule is "register as soon as you finish reading" — **after finishing the source of each className, immediately `register-code-read` once**. Phase 2 STEP A implements this (see SKILL-PHASE2.md "STEP A: read code → register-code-read").
...
If the same `/tmp/defect-detection/<repoHash>` clone is reused and HEAD has not changed, `ensure-gitnexus` skips `gitnexus analyze`. Do not run a second install probe after a successful clone-and-diff.
...
- Step 1.7 continues fetching requirement docs and extra tech docs from outside (doc-provider/doc-search)
...
1. **1.7.1**: Read tech docs & requirement docs (doc-provider → doc-search → web_fetch degrade)
...
doc-provider fails → doc-search → web_fetch → record `[FAILED]`; Phase 2 marks `[DOC_FETCH_FAILED]`
...
**`get-changed-methods` automatically**: parse git diff → extract method signatures/line numbers/bodyLineCount → call `set-diff-files` + `add-changed-methods` to persist.
...
**After Phase 1 completes → if you have not loaded `SKILL-PHASE2.md` yet, load it once and enter detection. Do not reload it after the filter.**
