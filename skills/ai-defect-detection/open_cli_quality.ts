...
import { _cli_result, _cli_success, _require_save } from "./open_cli_common.ts";
import { ContentStore, changed_method_name, content_base_dir } from "./open_store.ts";
import { _get_local_state_path, get_local_progress, get_all_batch_gits } from "./open_state.ts";
...
export function cmd_cross_view_check(args: Args): void {
  ...
  const resolved_directions = new Set<string>();

  if (!has_cases && directions_executed.includes("code_to_case_doc")) {
    resolved_directions.add("code_to_case_doc");
  }

  try {
    for (const ins of store.get_findings(null, "insight")) {
      const src = ins.source || "";
      if (src.startsWith("cross-view-")) {
        const tail = src.slice("cross-view-".length);
        for (const d of ["case_to_code", "doc_to_case", "doc_to_code", "code_to_case_doc"]) {
          if (tail.includes(d) || (ins.text || "").includes(d)) {
            resolved_directions.add(d);
          }
        }
      }
    }
  ...
  const critical_findings = findings.filter(
    (f) => f.actionRequired !== "review_orphan_change" && !resolved_directions.has(f.direction),
  );
  const gate_passed = critical_findings.length === 0;
  ...
  const result: Record<string, any> = {
    ...
    gate_passed,
    criticalCount: critical_findings.length,
    resolvedDirections: [...resolved_directions].sort(),
    byDirection: {},
  ...
  const gate_msg = gate_passed
    ? "Gate passed."
    : ("Gate failed: the agent must semantically verify each unpassed direction (program matching is only a first filter, " +
        "mixed Chinese/English rules false-positive easily); after verifying, use register-finding " +
        "--category insight --source cross-view-<direction> to leave a trace, " +
        "then re-run this command to pass the verified directions.");
  const resolved_label = resolved_directions.size ? [...resolved_directions].sort() : "none";
  const msg = (
    `Cross-view check finished: ran ${directions_executed.length} direction(s), ` +
    `found ${findings.length} coverage gap(s)` +
    ` (critical ${critical_findings.length}; semantically verified pass directions: ` +
    `${resolved_label}). ${gate_msg}`
  );
  _cli_result(gate_passed ? 0 : 0, result, msg);
}
...
export function cmd_finalize_all(args: Args): void {
  ...
  if (!args.batch_ids) {
    ...
    if (!batch_ids.length) {
      console.log(JSON.stringify({ code: -1, msg: "--batch-ids is empty and pending_cache has no batchId (run get-pending first)" }));
      process.exit(1);
    }
  ...
  const result = finalize_all(args.task_id, batch_ids);
  console.log(JSON.stringify(result, null, 2));
}
