...
import { class_names_match, has_code_been_read } from "./open_state.ts";
...
function _validate_rank(rank: Record<string, any>): void {
  ...
  if (bs != null && !_BUGSTATUS_ALLOWED.has(bs)) {
    throw new Error(`rank bugStatus illegal: ${bs}, allowed: 2, 6, 7`);
  }
}
...
export class ContentStore {
  ...
  add_changed_methods(class_name: string, methods: any[]): void {
    ...
    meta.diff.changedMethods[class_name] = list.map((m) => {
      ...
      return norm.bodyLineCount == null && !norm.params && !norm.signature
        ? norm.methodName
        : norm;
    });
    this._mark_dirty("meta");
  ...
  resolve_writeback_context(
    ...
    const batch_id = item ? item.batchId : null;
    const resolved_method = item ? item.methodName : method_name;
    const resolved_strategy = item ? item.strategyCode : strategy_code;
    const plan_file_path = item ? item.filePath : null;

    const svc = batch_id != null ? this.get_service_by_batch_id(batch_id) : null;
    ...
    if (plan_file_path && String(plan_file_path).startsWith("src/")) {
      file_path = plan_file_path;
      source = source + "→plan-filePath";
    } else {
      const class_as_file = class_name.split(".").join("/") + ".java";
      file_path = `src/main/java/${class_as_file}`;
      source = source + "→className-as-filePath";
    }
    ...
    return {
      ...
      resolved: Boolean(file_path && git_url),
      source,
    };
  }
...
export function get_unregistered_batch_classes(task_id: number, batch_id: number): string[] {
  const store = new ContentStore(task_id);
  const plan = store.load_plan_only();
  const unregistered: string[] = [];
  for (const item of plan.detectionPlan || []) {
    if (item.parentBatchId !== batch_id) continue;
    if (item.strategyCode === 8) continue;
    const class_name = item.className;
    if (!has_code_been_read(batch_id, class_name, task_id)) unregistered.push(class_name);
  }
  return [...new Set(unregistered)].sort();
}
...
if (process.argv[1] && resolve(process.argv[1]) === _this_file) {
  function _usage(): never {
    console.log(
      "Usage: node open_store.ts <task_id> [summary|plan|pending|errors|meta|context|writebacks|findings]",
    );
    process.exit(1);
  }
