...
> The SKILL-PHASE2.md trunk keeps only a compact decision flow; dimensions and rules live here.
...
3. **Credentials cannot be skipped** (core constraint 3 validation is the authority): even though reads are shared, **every className in the group must still `register-code-read`** (Hard rule 14), and **every strategy=11 method must still `register-context-read` with ≥2 records** (Hard rule 22). What is shared is "the read action and the analysis understanding", not "per-process registration credentials".
...
node "$SKILL_SCRIPT" register-context-read \
  ...
  --context-type "<caller|dependency|config|upstream|downstream>" \
  --context-path "<file path that was read>" \
  --context-desc "<brief note, e.g. 'OrderService#createOrder calls this method'>"
```
...
|------|------|--------|
| context-type | Context type | caller / dependency / config / upstream / downstream |
| context-path | Absolute path of the file that was read | string |
| context-desc | Short note of why it was read | ≤200 chars |
...
|------|------|----------|
| `[Confidence:HIGH]` | High-confidence defect | D9 completely indefensible; clear code evidence (e.g. an NPE path is reachable with no protection) |
...
1. Pick the matching template (A/B/C) from `SKILL-WRITEBACK.md`
...
`open_validate.ts` Hard rule 21 checks thinking content on writeback:
...
```bash
node "$SKILL_SCRIPT" check-analysis-quality \
  --task-id $TASK_ID --batch-id $BATCH_ID
```
