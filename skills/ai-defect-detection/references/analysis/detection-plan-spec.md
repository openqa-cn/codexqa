...
Execution mode is inferred automatically in Phase 1 `get-changed-methods` from total changed lines in `git diff --stat` (additions + deletions): **< 200 lines → light**, **≥ 200 lines → strict**. The mode field is injected into every detection-plan item and passed to open_validate.ts via `request_body["_mode"]`.
