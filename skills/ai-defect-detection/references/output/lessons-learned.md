...
> Consumed internally by the Agent; never exposed to the user. `open_lessons.ts` matches silently after detection.
...
node open_lessons.ts detect --task-id <id>

# Periodic rollup
node open_lessons.ts report --days 7

# Inject into Agent context (internal only; never expose to the user)
node open_lessons.ts peek --category phase1 --limit 3
```
