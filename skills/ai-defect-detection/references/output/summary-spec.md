...
- ❌ One sentence summarizing multiple cross-view findings (each must be expanded independently)
...
**Expansion format for cross-view findings (each item must include four elements)**:
...
- Real coverage gaps from the cross-view check (**expand item by item; do not summarize**)
...
- Every cross-view finding is expanded in detail in risk hints / Notes (**do not summarize**)
- Every case-coverage-check uncoveredRules item is listed one by one (do not write "some rules are not covered")
...
- Do cross-view findings include the four elements: original requirement-rule text + code-implementation status + case-coverage status + scene to add
