...
export class ArgumentParser {
  ...
  print_help(): void {
    const lines = [
      `usage: open_detect.ts [-h] [--token TOKEN] cmd ...`,
      "",
      this.description,
      "",
      "options:",
  ...
  print_subcommand_help(name: string): void {
    ...
    const lines = [
      `usage: open_detect.ts ${name} [-h] ${req}`,
      "",
      sub.help || "",
      "",
      "options:",
  ...
  print_usage(): void {
    console.error("usage: open_detect.ts [-h] [--token TOKEN] cmd ...");
  }

  fail(msg: string): never {
    this.print_usage();
    console.error(`open_detect.ts: error: ${msg}`);
    process.exit(2);
  }
}
...
export class SubParser {
  ...
  add_argument(flag: string, spec: Partial<ArgSpec> & { dest?: string; action?: string } = {}): void {
    this.sub.args.push({
      flags: [flag],
      dest: destFromFlag(flag, spec.dest),
      help: spec.help,
...
function coerce(raw: string, spec: ArgSpec): any {
  ...
  if (spec.type === "int") {
    ...
    if (Number.isNaN(value)) {
      console.error(`open_detect.ts: error: invalid int value: '${raw}'`);
      process.exit(2);
    }
  } else if (spec.type === "float") {
    value = parseFloat(raw);
    if (Number.isNaN(value)) {
      console.error(`open_detect.ts: error: invalid float value: '${raw}'`);
      process.exit(2);
    }
  }
  if (spec.choices && !spec.choices.includes(value)) {
    console.error(`open_detect.ts: error: invalid choice: '${raw}' (choose from ${spec.choices.join(", ")})`);
    process.exit(2);
  }
  return value;
}
