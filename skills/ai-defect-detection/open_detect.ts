...
import { set_token } from "./open_auth.ts";
import { AuthenticationError } from "./open_http.ts";
import {
...
} from "./open_cli_task.ts";

function build_parser(): ArgumentParser {
  const parser = new ArgumentParser({
    description: "AI defect-detection Skill trigger (modular)",
    epilog: "Examples:\n  node open_detect.ts submit-plan --plan-id 74873 --plan-type 2 --submit-user demo-user\n  node open_detect.ts status --task-id 42",
  });
  ...
  p_vs.add_argument("--summary", { required: true });
  ...
  p_us.add_argument("--summary", { required: true });
  ...
  sub.add_parser("get-tag-list", { help: "Get tag list" });
  ...
  p_c1.add_argument("--language", { default: "java" });
  ...
  p_c7.add_argument("--doc-type", { required: true, choices: ["techDocs", "prdDocs"] });
