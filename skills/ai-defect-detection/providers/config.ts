...
function _as_bool(value: any, defaultValue = false): boolean {
...
export function _parse_simple_yaml(text: string): Record<string, any> {
...
export function load_settings(config_path: string | null = null): Settings {
  ...
  const data_dir = resolve(
    process.env.DETECTION_DATA_DIR || raw.data_dir || join(SKILL_ROOT, "data"),
  );
  const enterprise_dir = resolve(
    process.env.DETECTION_ENTERPRISE_DIR || raw.enterprise_dir || join(SKILL_ROOT, "enterprise"),
  );
