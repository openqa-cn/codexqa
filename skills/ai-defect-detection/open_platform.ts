...
import { extract_user_id } from "./open_auth.ts";
import {
  auto_fill_ids,
  get_batch_git,
  is_already_written,
...
export function update_process(request_body: Record<string, any>): Record<string, any> {
  ...
  if (_py_bool(raw_method) && String(raw_method).includes("(")) {
    request_body = { ...request_body };
    request_body.methodName = raw_method.slice(0, String(raw_method).indexOf("("));
  }

  if (!_py_bool(request_body.parentBatchId)) {
  ...
  if (!_py_bool(request_body.detectTier)) {
    const _task_id = _py_or(request_body.taskId, 0);
    const _cn = _py_or(request_body.className, "");
    const _mn = _py_or(request_body.methodName, "");
    const _sc = request_body.strategyCode;
    if (_py_bool(_task_id) && _py_bool(_cn) && _py_bool(_mn) && _py_bool(_sc)) {
      const _plan_item = new ContentStore(_task_id).get_plan_item(_cn, _mn, _sc);
      if (_plan_item && _py_bool(_plan_item.detectTier)) {
        request_body = { ...request_body };
        request_body.detectTier = _plan_item.detectTier;
      }
    }
  }
...
export function batch_update_process(
  ...
  if (_py_bool(hollow_check.blocked)) {
  ...
  if (_py_bool(hollow_check.warnings)) {
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    if (res.code !== 0 && !_py_bool((_py_or(res.data, {}) as any).skipped)) {
  ...
  for (const entry of results) {
    ...
    if (entry.code === 0) {
      if (_py_bool(_py_get(_py_get(entry, "data", {}), "skipped"))) {
        skip_count += 1;
      } else {
        success_count += 1;
      }
...
export function finalize_rank(
  ...
  if (_py_bool(git_file_path)) body.gitFilePath = git_file_path;
  if (_py_bool(process_ids)) body.processIds = process_ids;
  if (_py_bool(task_id)) body.taskId = task_id;
...
const _SUMMARY_FORBIDDEN_RULES: Array<[string, string, string]> = [
  ...
  ["batch", "internal batch term 'batch'", "Use business wording such as 'service / module' and drop internal batch numbers"],
...
export function complete_task(
  ...
  if (_py_bool(summary_error)) {
    result._summary_status = "FAILED";
    result._summary_hint =
      `Detection summary upload failed: ${summary_error}.` +
      "Re-upload with `ai-defect-detection update-summary --task-id <taskId> --summary <correctly formatted summary>`.";
  } else if (_py_bool(detection_summary)) {
...
export function get_confirmed_defect_history(
  ...
  if (_py_bool(class_name)) params.className = class_name;
  if (_py_bool(git_file_path)) params.gitFilePath = git_file_path;
  if (_py_bool(method_name)) params.methodName = method_name;
...
export function preflight_complete_check(batch_ids: number[]): Record<string, any> {
  ...
  for (const bid of batch_ids) {
    ...
    if (cov_data.allCompleted === false || _py_bool(missed)) {
      blockers.push(
...
export function _norm_key(class_name: any, method_name: any): [string, string] {
...
export function reconcile_report(task_id: number): Record<string, any> {
  ...
  for (let b of String(_py_or(rdata.batchIds, "")).split(",")) {
    b = b.trim();
    if (_py_bool(b)) own_batches.add(b);
  }
  ...
  for (const job of rdata.jobDetectionVos || []) {
    ...
    if (_py_bool(own_batches) && !own_batches.has(job_bid)) continue;
  ...
  try {
    ...
    wb = typeof store._load_shard === "function" ? store._load_shard("writebacks") : store.data;
  ...
  if (_py_bool(missing_ranks_list)) {
    blockers.push(
  ...
  if (_py_bool(report_defects) && !_py_bool(rank_defect_keys)) {
    blockers.unshift(
  ...
  if (_py_bool(extra_list)) {
    ...
    for (const k of extra_in_report) {
      ...
      if (_py_bool(rk_bid) && !own_batches.has(rk_bid)) {
    ...
    if (_py_bool(drift_from_old_batch)) {
...
export function _humanize_inherited_feedback_result(result: string | null | undefined): string {
  return (
    {
...
export function _merge_rank_contents(defect_processes: Record<string, any>[]): string {
  ...
  let merged_content = _py_bool(contents) ? contents.join(" | ") : "See each strategy's detection conclusions for defect details";
  if (_py_bool(inherited_feedbacks)) {
    merged_content += "\n\n[Historical conclusions and human feedback]\n" + inherited_feedbacks.join("\n");
  }
  return merged_content;
}

export function finalize_all(task_id: number, batch_ids: number[]): Record<string, any> {
  ...
  for (const bid of batch_ids) {
    ...
    for (const item of missing || []) {
      const class_path = _py_get(item, "classPath", "");
      ...
      const proc_ids = (defect_procs || [])
        .filter((p: any) => _py_bool(p.processId))
      ...
      if (fr_result.code === 0 && _py_bool(task_id)) {
        try {
          ...
          if (_py_bool(rank_id)) rank_record.rankId = rank_id;
  ...
  const complete_result = complete_task(task_id);
...
export function _validate_report_content_for_task(task_id: number): Record<string, any> {
...
export function get_tag_list(): any[] {
  ...
  function _collect_all_tags(tag_list: any[], ids: Set<any>, mapping: Record<string, any>): void {
    for (const t of tag_list) {
      ...
      if (_py_bool(children) && Array.isArray(children)) {
