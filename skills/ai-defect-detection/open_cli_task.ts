...
import { _cli_result, _inject_strategy_codes } from "./open_cli_common.ts";
import { ContentStore } from "./open_store.ts";
import { _get_local_state_path } from "./open_state.ts";
  ...
  if (!force) {
    ...
    try {
      ...
      if ((has_cases || has_doc) && !cross_view_findings.length) {
        local_blockers.push(
          "Cross-view detection was not run: test cases or docs exist but findings has no cross-view record." +
            "Run Step 2.7c triangle cross-check before finalize.",
        );
      }
