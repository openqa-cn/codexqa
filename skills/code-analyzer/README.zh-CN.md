...
`code-analyzer` 用图证据回答的是**结构和影响面**问题；它不判断实现是否符合业务需求，也不产出 P0 / P1 / P2 审查结论。代码风险扫描报告用 `defect-detection`，playbook 驱动的具体实现问题用 `code-reviewer`，完整异常根因用 `root-cause-diagnosis`（在 CodexQA CLI 之上）。可以串联：先在这里圈影响面，再把深审或 RCA 聚焦到高风险路径。
