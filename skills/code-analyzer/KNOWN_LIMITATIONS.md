...
This file covers the limits of the `code-analyzer` skill and its required local analysis engine.
...
- No public host-agent run or independent quality benchmark has been published for `code-analyzer`.
...
`code-analyzer` maps structure, change impact, callers, entries, and test gaps. It does not execute tests, prove the absence of defects, decide whether code matches a business requirement, or replace P0 / P1 / P2 code review. Use `defect-detection` for requirement-oriented defect discovery and `code-reviewer` for playbook-driven implementation findings.
