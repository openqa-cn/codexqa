...
- [`root-cause-diagnosis`](root-cause-diagnosis/README.zh-CN.md)：在 CodexQA CLI 分析之上做异常根因诊断（堆栈/日志 → 带门禁的英文报告）。不是结构/影响面（`code-analyzer`），也不是需求写回（`defect-detection`）。[工作原理](root-cause-diagnosis/HOW_IT_WORKS.zh-CN.md)。
