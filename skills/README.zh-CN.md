...
- [`defect-detection`](defect-detection/README.zh-CN.md)：SAST/lint/secrets/SCA + agent 内联语义扫描 → `report_scan.*`（P0–P3）。不是 playbook CR（`code-reviewer`），不是结构/影响面（`code-analyzer`），也不是异常 RCA（`root-cause-diagnosis`）。[工作原理](defect-detection/HOW_IT_WORKS.zh-CN.md)。
...
- [`code-reviewer`](code-reviewer/README.zh-CN.md)：对本地 Git 工作副本（分支 / PR / commit）做 playbook 驱动的 P0/P1/P2 审查。它不是 SAST+agent 扫描流水线——那是 `defect-detection`。[工作原理](code-reviewer/HOW_IT_WORKS.zh-CN.md)。
