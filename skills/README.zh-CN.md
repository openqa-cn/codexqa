...
- [`codexqa-code-reviewer`](codexqa-code-reviewer/README.zh-CN.md)：CodexQA 图证据包 + 启发式维度 + Agent LLM judgment（去重）→ 双语 `REVIEW-REPORT.html`（PR/diff、全仓或 adhoc）。不是 SAST + Agent LLM Detection 扫描报告（`codexqa-defect-analyzer`），也不是单独的结构/影响面问答（`codexqa-code-analyzer`）。[工作原理](codexqa-code-reviewer/HOW_IT_WORKS.zh-CN.md)。
