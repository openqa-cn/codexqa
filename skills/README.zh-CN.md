...
安装某个 skill：`npx skills add openqa-cn/codexqa --skill <name>`（例如 `--skill skill-router` 或 `--skill defect-detection`）。

## 已发布的 skill

- [`skill-router`](skill-router/README.zh-CN.md)：对照现场兄弟与内置 catalog 匹配，必要时按需安装到路由旁边并交接。本身不干活——只选择、可拉取、再跟随其它 skill。[工作原理](skill-router/HOW_IT_WORKS.zh-CN.md)。
- [`defect-detection`](defect-detection/README.zh-CN.md)：SAST/lint/secrets/SCA + Agent LLM Detection → `report_scan.*`（P0–P3，去重合并）。不是图证据审查（`ai-code-reviewer`），不是结构/影响面（`code-analyzer`），也不是异常 RCA（`root-cause-diagnosis`）。[工作原理](defect-detection/HOW_IT_WORKS.zh-CN.md)。
...
- [`ai-code-reviewer`](ai-code-reviewer/README.zh-CN.md)：CodexQA 图证据包 + 启发式维度 + Agent LLM judgment（去重）→ 双语 `REVIEW-REPORT.html`（PR/diff、全仓或 adhoc）。不是 SAST + Agent LLM Detection 扫描报告（`defect-detection`），也不是单独的结构/影响面问答（`code-analyzer`）。[工作原理](ai-code-reviewer/HOW_IT_WORKS.zh-CN.md)。
