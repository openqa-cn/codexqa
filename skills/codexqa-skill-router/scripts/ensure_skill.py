#!/usr/bin/env python3
"""Ensure a routable codexqa skill is installed next to codexqa-skill-router.

When only codexqa-skill-router is installed, discover returns count 0. After the agent
...
Usage:
  python3 ensure_skill.py codexqa-code-reviewer --yes
  python3 ensure_skill.py codexqa-testcase-generator --dry-run
  python3 ensure_skill.py codexqa-defect-analyzer --from-repo /path/to/codexqa --yes
"""
import argparse
import json
import os
...
def install_from_tarball(name: str, dest_parent: Path, url: str) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="codexqa-skill-router-fetch-") as tmp:
        tar_path = Path(tmp) / "pack.tar.gz"
        req = urllib.request.Request(url, headers={"User-Agent": "codexqa-skill-router-ensure/1.0"})
...
def ensure(
    ...
    if name == SELF_NAME:
        return {"ok": False, "error": "refusing to install codexqa-skill-router into itself"}
    known = known_names()
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Ensure a codexqa skill is installed beside codexqa-skill-router")
