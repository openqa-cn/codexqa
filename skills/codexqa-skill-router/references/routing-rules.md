...
| 变更分析 / 变更代码分析 / 变更影响报告 / 影响入口 / 召回测试用例 / change impact report / change analysis, or impact words on one diff (这次 / PR / diff / 分支) plus an HTML report or 补上缺的测试 | `codexqa-change-analysis` | general symbol-graph Q&A (`codexqa-code-analyzer`), PRD case library |
