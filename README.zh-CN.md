...
**六个本地优先的 Agent Skills，覆盖需求、测试设计、测试数据、变更影响、需求缺陷和代码审查。**
...
<p align="center">
  <sub><em>Coding Agent 负责写出变更；codexqa 让意图、影响面、审查证据、用例和测试数据都能核查。<br>（上图是六类产物之一：使用同一渲染器和预置发现项的 defect-detection 样例页。）</em></sub>
</p>

---

AI 能很快产出一个绿 PR，但需求是否对齐、影响了谁、审查证据是否够、测试能否运行，仍然需要逐项确认。codexqa 把这些验证工作拆成六个 Agent Skills。
...
三个面向代码的 skill 会接触同一个仓库，但回答的问题不同：

| Skill | 核心问题 | 输入 | 它不替代什么 |
| --- | --- | --- | --- |
| [`code-analyzer`](skills/code-analyzer/README.zh-CN.md) | 改了什么、能打到哪里、有哪些测试缺口？ | 本地仓库 + 可选 diff 基线 | 需求语义判断或 P0 / P1 / P2 审查 |
...
各 Skill 的详细工作流和边界放在各自目录中：[代码分析](skills/code-analyzer/README.zh-CN.md)（[已知边界](skills/code-analyzer/KNOWN_LIMITATIONS.zh-CN.md)）、[缺陷检测](skills/defect-detection/HOW_IT_WORKS.zh-CN.md)、[代码审查](skills/code-reviewer/HOW_IT_WORKS.zh-CN.md)、[需求分析](skills/requirements-analyzer/HOW_IT_WORKS.zh-CN.md)、[用例生成](skills/testcase-generation/HOW_IT_WORKS.zh-CN.md)和[数据构造](skills/testdata-generation/HOW_IT_WORKS.zh-CN.md)。宿主、语言和验证状态统一见[支持矩阵](docs/SUPPORT_MATRIX.zh-CN.md)。

## 在 Cursor、Claude Code、Codex 上安装

先检查基础工具。`code-analyzer` 要求 Node.js 18+；`defect-detection` CLI 套件当前在 Node.js 22.15.0 上验证。
...
`code-analyzer` 还需要 Node.js 18+ 和 `npm install -g @openqa-cn/codexqa`。该 npm 包是单独分发的闭源本地代码分析引擎；建索引和查询在用户机器上完成，不需要 LLM。[详细边界](skills/code-analyzer/KNOWN_LIMITATIONS.zh-CN.md)。
...
这些检查覆盖文档链接、中英章节对齐、仓库内 `defect-detection` CLI 与 provider 行为、打包、任务隔离、写回校验和边界案例。它们不执行单独分发的 `code-analyzer` 引擎，也不能证明所有缺陷都会被发现。
